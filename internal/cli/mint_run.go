...
The dev mint replaces Secret Manager (stores PEMs on disk), Workload
...
Use --insecure-no-auth to disable OIDC verification (for local testing only).`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if insecureNoAuth {
				ip := net.ParseIP(bind)
				isLoopback := bind == "localhost" || (ip != nil && ip.IsLoopback())
				if !isLoopback {
					return fmt.Errorf("--insecure-no-auth requires a loopback bind address "+
						"(got %q); use --bind 127.0.0.1 or omit --bind", bind)
				}
			}

			if tunnel && len(allowedWorkflows) == 0 {
				return fmt.Errorf("--tunnel requires --allowed-workflows: a public tunnel with wildcard " +
					"workflow access lets any GitHub Actions workflow in the org mint tokens.\n" +
					"  To restrict to fullsend workflows: --allowed-workflows '.github/workflows/dispatch.yml'\n" +
					"  To allow all workflows (explicit opt-in): --allowed-workflows '*'")
			}
	...
	cmd.Flags().StringSliceVar(&allowedWorkflows, "allowed-workflows", nil, "restrict OIDC verification to specific workflow paths (default: all workflows)")
