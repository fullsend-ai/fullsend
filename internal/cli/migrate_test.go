...
func TestMigrateCustomizations_DeadOverride_WithAssociatedFiles(t *testing.T) {
	dir := t.TempDir()
	writeOrgConfig(t, dir, `agents:
  - source: "https://raw.githubusercontent.com/fullsend-ai/agents/abc123abc123abc123abc123abc123abc123abc1/harness/review.yaml#sha256=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
allowed_remote_resources:
...
`)
	setupCustomizedDir(t, dir, map[string]string{
		"harness/review.yaml":   "agent: agents/review.md\nmodel: opus\n",
		"agents/review.md":      "You are the review agent.\n",
	...
	// All associated files should be deleted, not moved.
	for _, f := range record.Files {
		assert.True(t, f.Delete, "dead override file %s should be deleted", f.Path)
	}
	...
	assert.Contains(t, deletePaths, "customized/harness/review.yaml")
	assert.Contains(t, deletePaths, "customized/agents/review.md")
	assert.Contains(t, deletePaths, "customized/scripts/pre-review.sh")
}
...
func TestBuildModifiedAgentFiles_DiffAbort(t *testing.T) {
	...
	fc := fakeClientWithRepo("my-org", ".fullsend")
	printer := ui.New(io.Discard)
	_, err := buildModifiedAgentFiles(context.Background(), customizedBase, "customized/", "", m, fc, cfg, printer)
	require.Error(t, err)
...
`)
	...
	fc := fakeClientWithRepo("my-org", ".fullsend")
	printer := ui.New(io.Discard)
	files, err := buildModifiedAgentFiles(context.Background(), customizedBase, "customized/", "", m, fc, cfg, printer)
	require.NoError(t, err)
...
func TestPlanMigrations_ExactStemBeforePrefixStrip(t *testing.T) {
	files := []string{
		"harness/review.yaml",
		"harness/pre-review.yaml",
		"scripts/pre-review.sh",
	}
...
`)
	...
	// scripts/pre-review.sh should associate with agent "pre-review"
	// (exact stem match), not "review" (prefix-stripped).
	preReviewFiles := make(map[string]bool)
	for _, f := range byName["pre-review"].files {
		preReviewFiles[f] = true
	}
	assert.True(t, preReviewFiles["scripts/pre-review.sh"],
		"pre-review.sh should match agent pre-review by exact stem")

	reviewFiles := make(map[string]bool)
	for _, f := range byName["review"].files {
		reviewFiles[f] = true
	}
	assert.False(t, reviewFiles["scripts/pre-review.sh"],
		"pre-review.sh should NOT match agent review via prefix stripping")
}
