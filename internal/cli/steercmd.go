...
func parseWorkItemURL(raw string) (workItemRef, error) {
	...
	switch {
	// Exactly github.com. A subdomain is not accepted: the host is discarded
	// after this switch and the client always talks to the github.com API, so
	// accepting `anything.github.com` would resolve and post to the same
	// owner/repo path on github.com instead of on the host the user named.
	// fullsend does not support GitHub Enterprise, so there is no host here
	// worth carrying forward rather than rejecting.
	case host == "github.com":
		...
		if err != nil {
			return workItemRef{}, fmt.Errorf("%w: %q", err, raw)
		}
		return workItemRef{Forge: repos.ForgeGitHub, Owner: owner, Repo: repo, Number: number}, nil

	case host == "gitlab.com" || strings.Contains(host, "gitlab"):
		// Recognized only so the error can name the real gap rather than
		// reporting an unknown host.
		return workItemRef{Forge: repos.ForgeGitLab}, nil
...
// workItemFromSegments reads the {owner}/{repo}/{kind}/{n} shape at the start
// of a URL path, where kind is one of kinds. Anything after the number (a
// Files tab, a review sub-path) is ignored. The shape is matched only at the
// start: searching the path for a kind segment would take the two segments
// before it as the repository, so /a/b/x/owner/repo/issues/7 would name
// owner/repo rather than being rejected.
func workItemFromSegments(segs []string, kinds map[string]bool) (string, string, int, string, error) {
	if len(segs) < 4 || !kinds[segs[2]] {
		return "", "", 0, "", fmt.Errorf("not a GitHub issue or pull request URL")
	}
	n, err := strconv.Atoi(segs[3])
	if err != nil || n <= 0 {
		return "", "", 0, "", fmt.Errorf("the segment after issues/pull is not a valid item number")
	}
	return segs[0], segs[1], n, segs[2], nil
}
...
func buildSteerComment(stage, text string, isPR bool) (string, error) {
	...
	if !isPR && stage != "triage" {
		return "", fmt.Errorf("--stage %q applies to a pull request, and the forge reports this number is an issue", stage)
	}
	return command + " " + text, nil
}

// newSteerForgeClientFn is the CLI's normal forge composition path. Override
// in tests to return a fake client instead of composing a live one.
var newSteerForgeClientFn = func() (forge.Client, error) {
...
Authentication uses GH_TOKEN, then GITHUB_TOKEN, then 'gh auth token'.`,
  ...
  fullsend steer --stage fix https://github.com/org/repo/pull/123 "rebase onto main first"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if item.Forge != repos.ForgeGitHub {
				return fmt.Errorf("steering is not supported on %q yet; "+
					"post a stage command such as /fs-review on the merge request by hand", item.Forge)
			}

			client, err := newSteerForgeClientFn()
			if err != nil {
				return err
			}
