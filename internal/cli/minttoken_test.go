...
func TestMintTokenCmd_InvalidLevel(t *testing.T) {
	cmd := newMintTokenCmd()
	cmd.SetArgs([]string{"--role", "triage", "--repos", "r", "--mint-url", "https://mint.example.com", "--level", "admin"})
	err := cmd.Execute()
	if err == nil {
		t.Fatal("expected error")
	}
	if !strings.Contains(err.Error(), "invalid --level") {
		t.Errorf("error = %q, want to contain 'invalid --level'", err.Error())
	}
}
