...
// (#6936).
//
// NOTE: TotalCostUSD is now reported by the SIGINT-into-sandbox
// mechanism (#6936): on cancellation, SIGINT is sent to the in-sandbox
// claude process, which flushes its ResultEvent (with cost data) before
// exiting. If the SIGINT delivery fails, TotalCostUSD falls back to
// zero — token counts are still captured via the deferred TokensEvent.
//
