...
func buildEnvScriptLines(sandboxName, remoteRepositoryDir string, h *harness.Harness, runtimeEnvExports []string, facts runFacts, fetchEnv ...fetchServiceEnv) []string {
	...
	// This stays the last line: unlike the static exports above it sources a
	// file rewritten before every iteration, so it must be re-read after
	// everything else.
...
//
// The lines are emitted after the .env.d sourcing and the env.sandbox block,
// so neither can shadow the values. They are runner-owned lines rather than
// env.sandbox entries or ${VAR} references: reservedSandboxKeys drops these
// keys from env.sandbox, and a ${FULLSEND_RUN_*} reference would hard-fail
// ValidateRunnerEnvWith on a host that does not define it.
