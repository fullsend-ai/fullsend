...
func downloadReleaseBinary(ver, arch, destPath string) error {
	...
	if _, err := io.Copy(&buf, io.LimitReader(resp.Body, maxDownloadSize+1)); err != nil {
		return fmt.Errorf("reading %s: %w", assetName, err)
	}
	if buf.Len() > maxDownloadSize {
		return fmt.Errorf("%s exceeds maximum download size (%d bytes)", assetName, maxDownloadSize)
	}
