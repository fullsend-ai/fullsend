...
func runAgent(ctx context.Context, agentName, fullsendDir, outputBase, targetRepo, fullsendBinary string, envFiles []string, noPostScript bool, debug string, forgeFlag string, rFlags resolveFlags, sOpts statusOpts, printer *ui.Printer, keepSandbox bool) (runErr error) {
	...
	// The service listens on a dynamic TCP port and authenticates requests
	// with a per-run bearer token. It remains idle unless an agent invokes
	// `fullsend fetch-skill`. PR 3 will gate this behind allow_runtime_fetch.
	var fetchEnvVal fetchServiceEnv
	var runtimeForgeClient forge.Client
	if rFlags.forgeClient != nil {
		runtimeForgeClient = rFlags.forgeClient
	} else if h.HasURLSkills() || len(h.AllowedRemoteResources) > 0 {
		if token, tokenErr := resolveToken(); tokenErr == nil {
			runtimeForgeClient = gh.New(token)
		}
	}
	fetchAddr, fetchToken, fetchShutdown, fetchErr := startFetchService(ctx, fetchsvc.ServiceConfig{
		Harness:       h,
		ForgeClient:   runtimeForgeClient,
	...
	if fetchErr != nil {
		...
		defer fetchShutdown()
		fetchEnvVal = fetchServiceEnv{addr: fetchAddr, token: fetchToken}
	}
...
func bootstrapEnv(sandboxName, remoteRepositoryDir string, h *harness.Harness, runtimeEnvExports []string, fetchEnv ...fetchServiceEnv) error {
	...
	if len(fetchEnv) > 0 && fetchEnv[0].addr != "" {
		...
		lines = append(lines, fmt.Sprintf("export FULLSEND_FETCH_TOKEN=%s", fetchEnv[0].token))
	}
