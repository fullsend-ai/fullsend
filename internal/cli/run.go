...
func buildClaudeCommand(agentName, model, repoDir string, pluginDirs []string, debug string) string {
	...
	if debug != "" {
		claudeParts = append(claudeParts, fmt.Sprintf("--debug-file '%s/%s'", sandbox.SandboxWorkspace, claudeDebugLog))
		if debug != "*" {
			claudeParts = append(claudeParts, fmt.Sprintf("--debug '%s'", strings.ReplaceAll(debug, "'", "'\\''")))
		}
	}

	if model != "" {
		claudeParts = append(claudeParts, fmt.Sprintf("--model '%s'", strings.ReplaceAll(model, "'", "'\\''")))
	}

	for _, pd := range pluginDirs {
		claudeParts = append(claudeParts, fmt.Sprintf("--plugin-dir '%s'", strings.ReplaceAll(pd, "'", "'\\''")))
	}
	claudeParts = append(claudeParts,
