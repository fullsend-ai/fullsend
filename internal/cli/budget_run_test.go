...
// actually started.
func useBudgetRunStub(t *testing.T, costPerIteration float64) string {
	t.Helper()
...
func TestRunAgent_BudgetHaltsValidationRetries(t *testing.T) {
	logPath := useBudgetRunStub(t, 0.06) // one iteration exhausts the 0.05 cap
...
func TestRunAgent_BudgetCrossedOnFinalIterationIsNotMarked(t *testing.T) {
	logPath := useBudgetRunStub(t, 0.06)
...
func TestRunAgent_UnderBudgetStillRetries(t *testing.T) {
	logPath := useBudgetRunStub(t, 0.01) // 3 iterations cost 0.03, well under the cap
