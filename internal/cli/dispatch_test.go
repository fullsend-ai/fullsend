...
func TestNewDispatchCmd_Flags(t *testing.T) {
	...
	forgeToken := cmd.Flags().Lookup("forge-token")
	require.NotNil(t, forgeToken)
	assert.Contains(t, forgeToken.Usage, "GH_TOKEN")
	assert.Contains(t, forgeToken.Usage, "GITHUB_TOKEN")
	assert.Contains(t, forgeToken.Usage, "gh auth token")
}

func TestRunDispatch_JSONDriverWithoutGitHubToken(t *testing.T) {
	t.Setenv("GH_TOKEN", "")
  ...
  "source": {"system": "github", "raw_type": "issues", "raw_action": "labeled"}
	...
	require.NoError(t, <-errCh, "json driver must not require GitHub credentials")
	assert.Contains(t, buf.String(), "issue-ping")
}

func TestRunDispatch_JSONDriverWithForgeToken(t *testing.T) {
	t.Setenv("GH_TOKEN", "")
	t.Setenv("GITHUB_TOKEN", "")
	t.Setenv("PATH", "/nonexistent")
