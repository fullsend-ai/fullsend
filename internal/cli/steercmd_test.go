...
func TestParseWorkItemURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: workItemRef{Forge: repos.ForgeGitHub, Owner: "org", Repo: "repo", Number: 123},
		},
		{
			name: "issue",
			...
			want: workItemRef{Forge: repos.ForgeGitHub, Owner: "org", Repo: "repo", Number: 7},
		},
		{
		...
		},
		{
			...
			want: workItemRef{Forge: repos.ForgeGitLab},
		},
		{
		...
		{
			name:    "an issues segment deeper in the path does not name the repository",
			url:     "https://github.com/display/repo/x/victim/target/issues/7",
			wantErr: "not a GitHub issue or pull request URL",
		},
		{
...
func withFakePoster(t *testing.T, p *fakePoster) {
	t.Helper()
	prev := newSteerForgeClientFn
	newSteerForgeClientFn = func() (forge.Client, error) {
		if p.clientErr != nil {
			return nil, p.clientErr
		}
		return p, nil
	}
	t.Cleanup(func() { newSteerForgeClientFn = prev })
...
func TestParseWorkItemURL_SelfHostedGitLab(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, repos.ForgeGitLab, got.Forge)
}
...
// TestParseWorkItemURL_RejectsGitHubSubdomains is a mis-targeting guard, not a
// style rule. parseWorkItemURL discards the host once it has decided the
// forge, and the client it returns always talks to the github.com API — so a
// subdomain that parsed successfully would resolve, and then POST A COMMENT,
// against the same owner/repo path on github.com rather than on the host the
// caller named. Rejecting is the only safe answer while there is nowhere to
// carry the host to.
func TestParseWorkItemURL_RejectsGitHubSubdomains(t *testing.T) {
	for _, raw := range []string{
		"https://gist.github.com/org/repo/pull/1",
		"https://enterprise.github.com/org/repo/issues/7",
		"https://notgithub.com/org/repo/pull/1",
	} {
		_, err := parseWorkItemURL(raw)
		require.Error(t, err, "%s must not resolve to a github.com work item", raw)
		assert.Contains(t, err.Error(), "unsupported forge host", raw)
	}

	ref, err := parseWorkItemURL("https://github.com/org/repo/pull/1")
	require.NoError(t, err, "the bare host must still work")
	assert.Equal(t, repos.ForgeGitHub, ref.Forge)
}
