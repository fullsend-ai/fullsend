...
func (p *Provisioner) EnsureOrgInMint(ctx context.Context, expectedURL string, org string) error {
	...
	if allowedOrgs == "" {
		...
		if raw := trafficEnvVars["ROLE_APP_IDS"]; raw != "" {
			if err := json.Unmarshal([]byte(raw), &roleAppIDMap); err != nil {
				return fmt.Errorf("data inconsistency: ALLOWED_ORGS is empty and ROLE_APP_IDS is present but not valid JSON: %w", err)
			}
		}
