...
func TestBuildWASM(t *testing.T) {
	...
	t.Run("includes cfaccess build tag when StatusCFAccess.Aud is set", func(t *testing.T) {
		...
		outPath := filepath.Join(t.TempDir(), "mintcore.wasm")
		err := buildWASM(outPath, "1.0.0", "abc", StatusGitHubAuth{}, StatusCFAccessAuth{Aud: "cf-aud-123", Team: "myteam"})
		require.NoError(t, err)
		require.NotNil(t, capturedCmd)

		args := strings.Join(capturedCmd.Args, " ")
		assert.Contains(t, args, "-tags cfaccess")
		assert.Contains(t, args, wasmLDFlags("1.0.0", "abc", StatusGitHubAuth{}, StatusCFAccessAuth{Aud: "cf-aud-123", Team: "myteam"}))
	})

	t.Run("includes both github and cfaccess build tags when both set", func(t *testing.T) {
		origExec := execCombinedOutputFn
		var capturedCmd *exec.Cmd
		execCombinedOutputFn = func(cmd *exec.Cmd) ([]byte, error) {
			capturedCmd = cmd
		...
		outPath := filepath.Join(t.TempDir(), "mintcore.wasm")
		err := buildWASM(outPath, "1.0.0", "abc", StatusGitHubAuth{Group: "acme/team"}, StatusCFAccessAuth{Aud: "cf-aud-123", Team: "myteam"})
		require.NoError(t, err)
		require.NotNil(t, capturedCmd)

		args := strings.Join(capturedCmd.Args, " ")
		assert.Contains(t, args, "-tags github,cfaccess")
	})
