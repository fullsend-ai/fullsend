		...
		t.Run(stage, func(t *testing.T) {
			...
			assert.Equal(t, "false", job.Concurrency.CancelInProgress.Value,
				"job %q must never cancel the run in flight when a newer event "+
					"arrives on the same work item (ADR 0106). A new stage job must "+
...
func TestPerRepoShimRunName(t *testing.T) {
	...
	// A comment-triggered run carries " comment:<id>" so a command can be
	// paired to its run exactly rather than by timestamp. The reader splits
	// at the first " comment:", so the separator, the token and the id's
	// position are all contract: a single space, lowercase, and last in the
	// title. Pinned literally because a drift in any of the three reads as
	// "no exact binding" rather than as an error.
	assert.Contains(t, shim.RunName, " comment:{0}",
		"the token is a single space then lowercase comment: — the reader splits on exactly that")
	assert.Contains(t, shim.RunName, "github.event.comment.id")
	assert.True(t, strings.HasSuffix(shim.RunName, "|| '' }}"),
		"the comment suffix must be last in the title; nothing may be appended after it")
}
