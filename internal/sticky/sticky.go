...
func NeutralizeHistory(oldBody string, cfg Config) string {
	...
	footer := ""
	if cfg.FooterMarker != "" {
		// The LAST occurrence is the real footer. Splitting on the first
		// would hand everything after an agent-authored copy of the footer
		// marker straight through undefanged.
		if idx := strings.LastIndex(rest, cfg.FooterMarker); idx >= 0 {
