...
func TestFakeClient_CompareChanges(t *testing.T) {
	f := NewFakeClient()
	f.Comparisons = map[string]*CommitComparison{"o/r/aaa...bbb": {Status: "ahead", Files: []ComparedFile{{Path: "x"}}}}
	cmp, err := f.CompareChanges(context.Background(), "o", "r", "aaa", "bbb")
	require.NoError(t, err)
