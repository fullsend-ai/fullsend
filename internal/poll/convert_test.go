...
func TestToNormalizedEvent_IssueNote(t *testing.T) {
	...
	mc.issue[1] = &Issue{IID: 1, AuthorID: 42} // actor is the author
	...
	if ne.Source.RawType != "issue_event" {
		t.Errorf("Source.RawType = %q, want %q", ne.Source.RawType, "issue_event")
	}
	...
	if ne.Actor.ID != "42" {
		t.Errorf("Actor.ID = %q, want %q", ne.Actor.ID, "42")
	}
...
func TestToNormalizedEvent_IssueLabel(t *testing.T) {
	...
	mc.issue[1] = &Issue{IID: 1, AuthorID: 99} // actor 55 is NOT the author
	p := newEventsPoller(mc)
	...
	if ne.Actor.ID != "55" {
		t.Errorf("Actor.ID = %q, want %q (resolved from label event)", ne.Actor.ID, "55")
	}
	if ne.Actor.Kind != "human" {
		t.Errorf("Actor.Kind = %q, want %q", ne.Actor.Kind, "human")
	}
...
func TestToNormalizedEvent_MRNote(t *testing.T) {
	...
	if ne.Source.RawType != "merge_request_event" {
		t.Errorf("Source.RawType = %q, want %q", ne.Source.RawType, "merge_request_event")
	}
	if ne.Transition.Kind != "comment_added" {
		t.Errorf("Transition.Kind = %q, want %q", ne.Transition.Kind, "comment_added")
	}
...
func TestToNormalizedEvent_MREventMerge(t *testing.T) {
	...
	event := RoutableEvent{
		Type:         "mr_event",
		IID:          5,
		NoteAuthorID: 10, // carries MergedByID
		IsBot:        false,
		MRSource:     1,
		MRTarget:     1,
	}
	...
	if ne.Actor.ID != "10" {
		t.Errorf("Actor.ID = %q, want %q", ne.Actor.ID, "10")
	}
...
func TestMapRawType(t *testing.T) {
	tests := []struct {
		...
		{"issue_label", "issue_event"},
		{"issue_note", "issue_event"},
		{"mr_note", "merge_request_event"},
...
func TestIsEntityAuthor_IssueNote(t *testing.T) {
	mc := newMockClient()
	mc.issue[1] = &Issue{IID: 1, AuthorID: 42}
	p := newEventsPoller(mc)
...
func TestIsEntityAuthor_MREvent(t *testing.T) {
	...
	// For MR events, isEntityAuthor always returns false.
	event := RoutableEvent{Type: "mr_event", IID: 5}
	if p.isEntityAuthor(context.Background(), event, 42) {
		t.Error("expected isEntityAuthor to return false for MR events")
	}
}
