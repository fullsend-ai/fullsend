...
func TestIsForkMR_SameProject(t *testing.T) {
	event := RoutableEvent{MRSource: 1, MRTarget: 1}
	if isForkMR(event) {
		t.Error("expected isForkMR to return false for same project")
	}
}

func TestIsForkMR_DifferentProjects(t *testing.T) {
	event := RoutableEvent{MRSource: 1, MRTarget: 2}
	if !isForkMR(event) {
		t.Error("expected isForkMR to return true for different projects")
	}
}

func TestIsForkMR_ZeroValuesDenyByDefault(t *testing.T) {
	tests := []struct {
		name     string
		source   int
		target   int
		...
		{"zero source", 0, 1, true},
		{"zero target", 1, 0, true},
		{"both zero", 0, 0, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			event := RoutableEvent{MRSource: tt.source, MRTarget: tt.target}
			got := isForkMR(event)
			if got != tt.wantFork {
				t.Errorf("isForkMR(source=%d, target=%d) = %v, want %v",
					tt.source, tt.target, got, tt.wantFork)
			}
		})
	}
}
