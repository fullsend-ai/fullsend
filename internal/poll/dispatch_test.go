...
func TestDispatch_BuildsPayloadAndAppends(t *testing.T) {
	...
	if d.ResourceKey != "issue_comment-42" {
		t.Errorf("resource_key: got %q, want issue_comment-42", d.ResourceKey)
	}
...
func TestGenerateChildPipelineYAML_EmptyDispatches(t *testing.T) {
	yaml := generateChildPipelineYAML(nil)
	if yaml != "" {
		t.Errorf("expected empty string, got %q", yaml)
	}

	yaml = generateChildPipelineYAML([]Dispatch{})
	if yaml != "" {
