...
func TestRunFullPollWithRouterAndDispatch(t *testing.T) {
	...
	mc.issues = []Issue{
		{IID: 1, Labels: []string{"bug"}, UpdatedAt: now, AuthorID: 42},
	}
	mc.notes[1] = []Note{
...
func TestRunMultipleStages(t *testing.T) {
	...
	mc.variables["FULLSEND_LAST_POLL_AT_FULL"] = since.Format(time.RFC3339)
	mc.issues = []Issue{
		{IID: 1, Labels: []string{"ready-to-code"}, UpdatedAt: now, AuthorID: 5},
	}
	mc.notes[1] = []Note{}
	mc.issue[1] = &Issue{IID: 1, AuthorID: 5}
...
func TestRunLabelFailureRollback(t *testing.T) {
	...
	mc.issues = []Issue{
		{IID: 1, Labels: []string{"ready-to-code"}, UpdatedAt: now, AuthorID: 5},
		{IID: 2, Labels: []string{"bug"}, UpdatedAt: now, AuthorID: 42},
	}
	mc.notes[1] = []Note{}
	mc.notes[2] = []Note{
	...
	mc.memberLevel[42] = 30
	mc.issue[2] = &Issue{IID: 2, AuthorID: 42}
