...
// codexTurnTimeout is what one process of a codex run gets: the run's
// budget, clipped to whatever is left of the context's deadline. A steered
// run is several processes sharing one budget, so a turn must not restart
// the clock — Run puts a single deadline on the context and this reads it.
// An exhausted budget is an error rather than a zero timeout, which the
// sandbox would read as "no limit".
func codexTurnTimeout(ctx context.Context, budget time.Duration) (time.Duration, error) {
	// A cancelled context can still be short of its deadline; starting a
	// process under it only has exec kill it again, and the caller would
	// read that as a failed resume.
	if err := ctx.Err(); err != nil {
		return 0, fmt.Errorf("the run's context ended (%v); not starting another codex turn", err)
	}
	...
	if budget <= 0 {
		return 0, fmt.Errorf("the run's budget is spent; not starting another codex turn")
	}
	return budget, nil
}
...
func buildCodexTurnCommand(params RunParams, model, effort string, hooksEnabled bool, digests codexRunnerHeldDigestSet, turn codexTurn) string {
	...
	if turn.ResumeThreadID != "" {
		...
		// only on `codex exec` itself. The composed form was run locally on
		// 0.152.1 and 0.157.0 and parsed through to reading the prompt from
		// stdin.
...
func (r CodexRuntime) Run(ctx context.Context, params RunParams, printer *ui.Printer, start time.Time, metrics *RunMetrics) (int, error) {
	...
	if params.Steerable {
		q = newCodexSteerQueue(params.SandboxName, os.Stderr)
		registerCodexSteerQueue(params.SandboxName, q)
		// Settle before it leaves the registry, so a steer that already
		// looked the queue up is refused instead of joining an abandoned one.
		defer func() {
			q.settle()
			unregisterCodexSteerQueue(params.SandboxName)
		}()
		defer func() { metrics.Steers = q.steerResults() }()

		// A steered run is several processes, and params.Timeout is the
		// budget for the RUN, not for each of them. Without one deadline
		// over the loop, N resumes would get N times the budget and a
		// steered run could outlast the credential that has to post its
		// result. WithDeadline keeps whichever bound is earlier, so a
		// caller that already set one still wins.
	...
	var (
		...
		// lastGood is the newest outcome that actually ran a turn. A resume
		// that never opened a thread delivered nothing and its outcome
		// describes that failure, not the run — reporting it would turn a
		// run whose work completed into a failure because a later steer
		// could not be delivered. The steer is abandoned to the queued run;
		// the run itself keeps the verdict it earned.
		lastGood *codexTurnOutcome
		turn     codexTurn
		nth      int
		emitted  bool
	)
	for {
		if q != nil {
			...
			// stop with nothing to end.
			q.beginTurn()
		}
		out, err = r.runCodexTurn(ctx, params, printer, plan, turn, metrics, agg, q, nth, &emitted)
		stoppedForSteer := false
		if q != nil {
			// Nothing is running from here until the next beginTurn, so a
			// steer arriving now must not stop the sandbox.
			q.endTurn()
			// A turn that completed on its own before the stop reached it is
			// not an interrupted turn, whatever the mark says: Steer can take
			// q.mu between the process exiting and the loop's endTurn, and
			// stop a sandbox with nothing left running in it.
			stoppedForSteer = q.takeStoppedForSteer() && !turnCompleted(out)
		...
		if out.exitCode == codexHooksMissingExit || out.exitCode == codexConfigTamperedExit {
			// The integrity guards refused to start codex. That is the
			// run's verdict whatever came before: it is not a resume that
			// missed, to be retried and then dropped, and an earlier turn's
			// success must not stand in for it.
			q.settleIfSteerable()
			return r.codexVerdict(out, printer)
		}
		if err != nil {
			// Settle at the moment the run commits to stopping, not when
			// the deferred cleanup eventually runs: until it is set, a
			// concurrent Steer still enqueues and returns nil for a queue
			// nothing will drain. This narrows that window to the lock; it
			// cannot close it, which is why a nil return from Steer means
			// "accepted", not "delivered" (see runtime.Steerer).
			q.settleIfSteerable()
			if lastGood != nil {
				// A turn already completed; this error is a failed resume,
				// which costs the steer, not the run.
				printer.StepWarn("A steered resume failed after a turn had already completed; reporting that turn's result: " + err.Error())
				return r.codexVerdict(*lastGood, printer)
			}
			return out.exitCode, err
		}
		if stoppedForSteer {
			// The relay-closed exit of a process the runner stopped itself.
			// It ran a turn but did not finish it, so it is not lastGood;
			// the resume that follows carries the run on.
			printer.StepInfo("Stopped the codex turn to deliver a steer; resuming the same thread")
		} else if out.threadID != "" {
			completed := out
			lastGood = &completed
		}
		if q == nil {
		...
		if q.currentThreadID() == "" {
			// No process has opened a thread, so there is nothing to resume
			// onto. Waiting for a steer would only spend the budget; this
			// process's outcome is the run's verdict.
			break
		}
	...
	// Same reason as the error path above: stop accepting before the
	// verdict is computed rather than at unwind.
	q.settleIfSteerable()
	final := out
	if out.threadID == "" && lastGood != nil {
		// The loop ended on a resume that never opened a thread — the
		// abandoned-steer path. Report the turn that ran, not the one that
		// did not.
		final = *lastGood
	}
	return r.codexVerdict(final, printer)
}

// turnCompleted reports whether a process finished its turn on its own:
// a clean exit and a result that is not an error.
func turnCompleted(out codexTurnOutcome) bool {
	return out.exitCode == 0 && out.lastResult != nil && !out.lastResult.IsError
}
...
func nextCodexTurn(ctx context.Context, q *codexSteerQueue) (codexTurn, bool) {
	for {
		if ctx.Err() != nil {
			// Checked before anything is taken, so what is queued stays
			// queued and undelivered: no acknowledgement is recorded, and
			// the queued run redoes the work. Resuming here would start a
			// process the ended context kills at once, and count it as a
			// failed resume against the steer.
			return codexTurn{}, false
		}
		if msg, failures, ok := q.takePending(); ok {
			tid := q.currentThreadID()
			if tid == "" {
				// takePending already removed it, so returning here without
				// putting it back would discard a steer Steer accepted.
				// There is no thread to resume, so this run cannot deliver
				// it — but it must not vanish from the queue on the way out.
				q.requeueFront(msg, failures)
				return codexTurn{}, false
			}
			q.stakeDelivery(msg, failures, time.Now())
...
func (r CodexRuntime) runCodexTurn(
	...
	timeout, err := codexTurnTimeout(ctx, params.Timeout)
	if err != nil {
		return outcome, err
	}
	...
	handler = func(evt AgentEvent) {
		...
		switch e := evt.(type) {
		case ResultEvent:
			if e.IsError && q.stopRequested() {
				// The runner stopped the sandbox under this process to
				// deliver a steer, so the stream ended without a completed
				// turn. That is the runner's own interrupt, not an agent
				// failure: nothing is rendered or folded for it, and the
				// loop reports the interrupt instead.
				return
			}
