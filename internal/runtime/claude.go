...
// claudeSteerLoaderEnvUnset clears, right after the agent-writable .env is
// sourced, the variables that change what the pinned claude binary loads:
// NODE_OPTIONS/NODE_PATH for the npm install, LD_* for the dynamic loader.
// The binary pin alone does not stop a .env exporting NODE_OPTIONS=--require
// from running code inside the real claude, which could forge the
// --replay-user-messages echo the runner records as delivery. Same list as
// the codex launch's loader hygiene. Only the steerable launch carries it:
// that is where Claude's echo becomes delivery proof, and the ordinary
// launch keeps honouring a .env that sets these on purpose.
const claudeSteerLoaderEnvUnset = "unset NODE_OPTIONS NODE_PATH LD_PRELOAD LD_LIBRARY_PATH LD_AUDIT"
...
func buildRunCommand(params RunParams) string {
	...
	if params.Steerable {
		...
		// load-bearing.
		launch = fmt.Sprintf("cd %s && %s && %s && . %s && %s && %s | \"$%s\"",
			params.RepoDir, steerPathPin(), claudeBinaryPin(), envFile, claudeSteerLoaderEnvUnset,
