...
func TestParseClaudeStreamUserReplayAck(t *testing.T) {
	...
	// Content is what the ack is matched on, so a parser that dropped or
	// truncated it would leave every echo matching nothing outstanding and
	// the run would never settle on a delivery.
	if ack.Content != "STEER: also check the error path" {
		t.Errorf("expected the consumed message verbatim, got %q", ack.Content)
	}
}
