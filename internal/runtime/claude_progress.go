...
func parseClaudeStream(r io.Reader, onEvent func(AgentEvent)) error {
	return parseClaudeStreamLines(r, onEvent, nil)
}
...
// that map to no AgentEvent — and for every fully consumed line too large to
// parse (> streamBufSize). Any stream activity is proof of life, so the stall
// watchdog counts lines, not just the sparser semantic events.
