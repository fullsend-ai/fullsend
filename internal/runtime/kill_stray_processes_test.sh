...
# With no argument, exercise BOTH renderings: the interrupt one is the codex
# steer path, and a run that only covered the default would prove nothing
# about the rendering steering actually depends on.
if [ "$#" -eq 0 ]; then
  for snippet in "${SCRIPT_DIR}/testdata/kill_stray_processes.sh" \
                 "${SCRIPT_DIR}/testdata/kill_stray_processes_interrupt.sh"; do
    echo "=== ${snippet##*/} ==="
    bash "${BASH_SOURCE[0]}" "$snippet"
  done
  exit 0
fi
