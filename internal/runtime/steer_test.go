...
// TestSteerFeedMailboxWrites_ExecuteNoPayload runs the mailbox writes the
// runner actually sends (seed's `>` and appendLine's `>>`) under a real sh,
// with hostile steer text and a mailbox path that holds a space and a
// single quote. A payload that broke out of its quoting would create the
// canary; one that was reinterpreted would change the bytes written. The
// feed's own methods render the command, so an edit to how either write is
// built is tested where it lives, not against a restated format string.
func TestSteerFeedMailboxWrites_ExecuteNoPayload(t *testing.T) {
	sh, err := exec.LookPath("sh")
	if err != nil {
		t.Skip("no sh on PATH")
	}
	dir := t.TempDir()
	configDir := filepath.Join(dir, "steer cfg's")
	require.NoError(t, os.Mkdir(configDir, 0o755))
	canary := filepath.Join(dir, "PWNED")

	payload := "it's '; touch " + canary + "; echo ' $(touch " + canary + ") `touch " + canary + "` %s %n \\ \"q\""
	encoded, err := json.Marshal(map[string]string{"type": "prompt", "message": payload})
	require.NoError(t, err)

	run := func(ctx context.Context, _ string, cmd string, _ time.Duration) (string, string, int, error) {
	...
	f := newSteerFeed("sbx", configDir, run)
	ctx := context.Background()
	require.NoError(t, f.seed(ctx, payload, "seed-key"))
	require.NoError(t, f.appendLine(ctx, SteerMessage{FollowUpRunID: 1}, string(encoded), "k1"))
	require.NoError(t, f.appendLine(ctx, SteerMessage{FollowUpRunID: 2}, payload+" second", "k2"))

	if _, err := os.Stat(canary); !os.IsNotExist(err) {
		t.Fatalf("a mailbox write executed part of its payload (canary stat: %v)", err)
	}
	got, err := os.ReadFile(filepath.Join(configDir, steerMailboxName))
	require.NoError(t, err)
	want := payload + "\n" + string(encoded) + "\n" + payload + " second\n"
	assert.Equal(t, want, string(got), "the mailbox must hold every line byte-for-byte")
}
