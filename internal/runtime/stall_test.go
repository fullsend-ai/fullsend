...
// `user` tool_result lines are stream activity and must feed the watchdog
// through the line hook, whatever AgentEvent they also map to.
func TestParseClaudeStream_ToolResultLinesResetTheSilenceClock(t *testing.T) {
	...
	require.Len(t, events, 1)
	assert.IsType(t, ToolResultEvent{}, events[0])
	assert.Positive(t, w.lastEvent.Load(),
