...
func (PiRuntime) Steer(ctx context.Context, sandboxName string, msg SteerMessage) error {
	if reason, declined := piSteerDeclines.Load(sandboxName); declined {
		return fmt.Errorf("%w: %s", ErrSteerUnsupported, reason)
	}
...
// piSteerDeclines maps a sandbox name to the reason its running pi run
// declined steering, so Steer reports ErrSteerUnsupported and the runner
// leaves the update to the queued run instead of retrying a session that
// will never register.
var piSteerDeclines sync.Map // sandboxName -> string

// piSteerFallbackReason is why a pi run that can fall back across models is
// not steerable. The session id and the mailbox are bound to one pi launch,
// and a fallback relaunches pi on the next model after clearing the
// sessions dir. Model fallback wins: it keeps a run alive when Vertex does
// not serve the primary model, while a missed steer only costs the queued
