...
// steerPathVar holds the PATH captured before the agent-writable .env is
// sourced, so the feeder resolves `tail` through a value .env cannot reach.
// Same defence, and the same reason, as codexPathVar.
const steerPathVar = "FULLSEND_STEER_PATH"

// steerPathPin captures PATH for the feeder. It must be emitted BEFORE the
// .env is sourced. `readonly` means a .env assigning the same name aborts
// the sourcing shell under a POSIX sh rather than winning.
func steerPathPin() string { return "readonly " + steerPathVar + `="$PATH"` }
...
//
// This fragment runs after .env is sourced and whatever it executes owns
// the channel the runner delivers steers on, so every name in it is pinned
// two ways.
//
// PATH is the captured one, so a directory planted ahead of it cannot
// supply `tail`. That alone is not enough: a shell FUNCTION beats a PATH
// lookup and beats a regular builtin too — only special builtins outrank a
// function — so all three names are unset first. Measured, not assumed: a
// .env defining `tail`, `echo` or `wait` as a function wins every time
// without this. Each is a distinct primitive — a planted `tail` drops real
// steers or feeds a forged one, a planted `wait` writes its own stdout
// straight into the agent's stdin, and a planted `echo` corrupts the pid
// file so the feeder is never killed. `unset -f` on a name that is not a
// function succeeds, so this costs nothing when .env is well-behaved.
func steerFeederFragment(mailboxPath, pidPath string) string {
	return fmt.Sprintf("{ unset -f tail echo wait; PATH=\"$%s\" tail -n +1 -f %s & echo $! > %s ; wait ; }",
		steerPathVar, shellQuote(mailboxPath), shellQuote(pidPath))
}
