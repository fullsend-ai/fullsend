...
func TestPiRuntimeProviderFor(t *testing.T) {
	...
	t.Run("cross-runtime provider divergence", func(t *testing.T) {
	...
	// Edge cases: malformed specs may produce empty from ProviderFor;
	// GenAISystemFor must fall back to System() rather than stamping "".
	t.Run("malformed specs fall back to System", func(t *testing.T) {
		t.Setenv(piProviderEnv, "")
		assert.NotPanics(t, func() { rt.ProviderFor("//", "", nil) })
		assert.NotPanics(t, func() { rt.ProviderFor("/model", "", nil) })
		// GenAISystemFor catches the empty return and uses System().
		assert.Equal(t, "pi", GenAISystemFor(rt, "//", "", nil),
			"malformed spec should fall back to System(), not empty")
	})
}
