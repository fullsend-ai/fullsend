...
// complete installation on the given route. Vertex is the historical
// list (requiredSecretsForForge). Under openai the GCP pair is not part
// of the install: GitHub needs FULLSEND_OPENAI_API_KEY unless the config
// carries the OpenAI WIF trio; GitLab, where WIF is unavailable, needs
// the project's own OPENAI_API_KEY variable (probed, never managed) and
// still the forge token.
func requiredSecretsForRoute(forgeName string, route InferenceRoute) []string {
	if !route.OpenAI() {
		return requiredSecretsForForge(forgeName)
	}
	if forgeName == ForgeGitLab {
		return []string{forge.SecretForgeToken, gitlabOpenAIKeyVar}
	}
	if route.OpenAIWIF {
		return nil
	}
