...
// validated by the status endpoint. Only fields that are actually
// read during validation are declared; additional CF Access claims
// (type, sub) are intentionally omitted.
...
//  2. Checks issuer, audience, and token timestamps.
//  3. Validates the JWT signature against the CF Access JWKS endpoint.
//
// This file is compiled only when the cfaccess build tag is active.
// StatusCFAccessAud and StatusCFAccessTeam are always stamped at
// build/deploy time when the tag is enabled — empty values are not
// used as a runtime feature toggle.
//
// Returns nil on success. Returns errStatusAuthSkip when no
// Cf-Access-Jwt-Assertion header is present.
...
func (c *cfAccessKeyCache) getKey(ctx context.Context, kid, jwksURL string) (*rsa.PublicKey, error) {
	...
	expired := time.Since(fetchedAt) > jwksCacheTTL
	...
	if err != nil {
		if ok && time.Since(fetchedAt) <= maxKeysStaleness {
			return key, nil
		}
		return nil, err
	}
