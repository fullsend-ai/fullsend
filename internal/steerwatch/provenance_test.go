...
func TestRouteVerdict(t *testing.T) {
	...
	running := forge.WorkflowRun{Status: "in_progress"}
	completed := forge.WorkflowRun{Status: "completed", Conclusion: "skipped"}
	ok, pending, noRoute := routeVerdict(completed, jobs)
	assert.True(t, ok)
	assert.False(t, pending)
	assert.False(t, noRoute)

	jobs[0].Conclusion = "failure"
	ok, pending, _ = routeVerdict(completed, jobs)
	assert.False(t, ok)
	assert.False(t, pending, "a concluded failure is a final answer")

	// "Not yet" must be distinguishable from "no": these become an answer
	// on a later poll, and treating them as refusals would discard a
	// legitimate update for being polled a moment early.
	jobs[0].Conclusion = ""
	ok, pending, _ = routeVerdict(running, jobs)
	assert.False(t, ok)
	assert.True(t, pending, "a Route job still running has not answered")

	ok, pending, noRoute = routeVerdict(running, []forge.WorkflowJob{{Name: stageName}})
	assert.False(t, ok)
	assert.True(t, pending, "no Route job listed yet on a running run is not a refusal")
	assert.False(t, noRoute)

	// The run every status comment fires: the shim's own `if` skipped the
	// event, the reusable workflow was never called, and the listing is
	// stop-fix plus the bare dispatch wrapper, both skipped. Completed and
	// Route-less is final, or the watcher re-examines it until the deadline.
	skipped := []forge.WorkflowJob{
		{Name: "stop-fix", Status: "completed", Conclusion: "skipped"},
		{Name: "dispatch", Status: "completed", Conclusion: "skipped"},
	}
	ok, pending, noRoute = routeVerdict(completed, skipped)
	assert.False(t, ok)
	assert.False(t, pending, "a completed run will never gain a Route job")
	assert.True(t, noRoute)
}
...
func TestBoundToItem(t *testing.T) {
	...
	// A run the forge already associated with other pull requests is
	// another item's run: its title never gets a say.
	foreign := forge.WorkflowRun{PullRequestNumbers: []int{8}, DisplayTitle: "org/repo#7"}
	assert.False(t, boundToItem(foreign, "org/repo#7", 7))
}
...
func TestJobChecks(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "a completed run the shim skipped is rejected for good",
			jobs:       jobsJSON(stageJob("stop-fix", "completed", "skipped"), stageJob("dispatch", "completed", "skipped")),
			status:     "completed",
			conclusion: "skipped",
			wantCheck:  "route",
			wantDetail: "no Route job: the shim did not dispatch this event",
		},
		{
			name:        "a running run with no Route job yet is pending",
			jobs:        jobsJSON(stageJob("dispatch", "in_progress", "")),
			status:      "in_progress",
			wantCheck:   "route",
			wantPending: true,
		},
		{
			name:       "a completed run without my stage job is final",
			jobs:       jobsJSON(routeJob("success"), stageJob("dispatch / Triage", "completed", "success")),
			status:     "completed",
			conclusion: "success",
			wantCheck:  "stage",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.status != "" {
				run.Status, run.Conclusion = tt.status, tt.conclusion
			}
			...
			require.NotNil(t, rej)
			assert.Equal(t, tt.wantCheck, rej.check, rej.String())
			assert.Equal(t, tt.wantPending, rej.pending, rej.String())
			if tt.wantDetail != "" {
				assert.Equal(t, tt.wantDetail, rej.detail)
			}
		})
	}
}
...
func TestResolveStageJob(t *testing.T) {
	...
	// The dispatch workflow's Harness dispatch job is still running when a
	// stage job's watcher starts. The slug never names the stage job, so the
	// job id (GITHUB_JOB) has to.
	t.Run("the job id picks the stage job beside Harness dispatch", func(t *testing.T) {
		for _, tt := range []struct{ jobID, slug, want string }{
			{"review", "fullsend-ai-review", "dispatch / Review"},
			{"fix", "fullsend-ai-coder", "dispatch / Fix"},
		} {
			jobs := []forge.WorkflowJob{
				{Name: "dispatch / Harness dispatch", Status: "in_progress"},
				{Name: "dispatch / Route", Status: "completed", Conclusion: "success"},
				{Name: "dispatch / Triage", Status: "completed", Conclusion: "skipped"},
				{Name: "dispatch / Code", Status: "completed", Conclusion: "skipped"},
				{Name: tt.want, Status: "in_progress"},
			}
			name, err := resolveStageJob(jobs, tt.jobID, tt.slug)
			require.NoError(t, err, tt.jobID)
			assert.Equal(t, tt.want, name)
		}
	})

	t.Run("a matrix job falls through the job id to the agent hint", func(t *testing.T) {
		jobs := []forge.WorkflowJob{
			{Name: "dispatch / Harness dispatch", Status: "completed", Conclusion: "success"},
			{Name: "dispatch / Harness run (reviewer)", Status: "in_progress"},
			{Name: "dispatch / Harness run (fixer)", Status: "in_progress"},
		}
		name, err := resolveStageJob(jobs, "harness-run", "fixer")
		require.NoError(t, err)
		assert.Equal(t, "dispatch / Harness run (fixer)", name)
	})

	// GITHUB_JOB is the job id `harness-run` while the API reports the
	// display name `Harness run (<agent>)`: the hyphen and the space must
	// compare equal, or the id never matches a matrix job at all.
	t.Run("the job id matches the matrix job's display name", func(t *testing.T) {
		jobs := []forge.WorkflowJob{
			{Name: "dispatch / Harness dispatch", Status: "in_progress"},
			{Name: "dispatch / Harness run (x)", Status: "in_progress"},
			{Name: "dispatch / Harness run (y)", Status: "completed", Conclusion: "success"},
		}
		name, err := resolveStageJob(jobs, "harness-run", "slug-not-in-any-name")
		require.NoError(t, err)
		assert.Equal(t, "dispatch / Harness run (x)", name)
	})

	t.Run("two fan-out jobs in progress need the agent hint", func(t *testing.T) {
		jobs := []forge.WorkflowJob{
			{Name: "dispatch / Harness run (x)", Status: "in_progress"},
			{Name: "dispatch / Harness run (y)", Status: "in_progress"},
		}
		name, err := resolveStageJob(jobs, "harness-run", "y")
		require.NoError(t, err)
		assert.Equal(t, "dispatch / Harness run (y)", name)
		_, err = resolveStageJob(jobs, "harness-run", "slug-not-in-any-name")
		require.Error(t, err, "the id matches both; a slug absent from every name resolves nothing")
	})

	t.Run("hints that match nothing fail closed", func(t *testing.T) {
		jobs := []forge.WorkflowJob{
			{Name: "dispatch / Harness dispatch", Status: "in_progress"},
			{Name: "dispatch / Review", Status: "in_progress"},
		}
		_, err := resolveStageJob(jobs, "fix", "fullsend-ai-coder")
		require.Error(t, err)
...
func TestRunCreatedAt(t *testing.T) {
	...
	assert.Equal(t, 2026, runCreatedAt(forge.WorkflowRun{CreatedAt: "2026-09-03T10:00:00.123Z"}).Year(), "fractional seconds are accepted")
...
func TestActorLogin(t *testing.T) {
	// `actor` is the event's initiator — the comment author on an
	// issue_comment run — and is what the Route job authorized.
	// `triggering_actor` names whoever caused the latest attempt, so on a
	// re-run it is the person who pressed the button; it confers nothing.
	assert.Equal(t, "reviewer", actorLogin(forge.WorkflowRun{Actor: "reviewer", TriggeringActor: "reviewer"}))
	assert.Equal(t, "reviewer", actorLogin(forge.WorkflowRun{Actor: "reviewer", TriggeringActor: "rerunner", RunAttempt: 2}))
	assert.Empty(t, actorLogin(forge.WorkflowRun{TriggeringActor: "rerunner"}))
}
...
// A comment made while my job was still queued produces a follow-up that
// isFollowUp accepts; the first delta must start at the same instant, or
// that comment is filtered out, its run left unbound, and the queued run
// redoes the work.
func TestFirstDelta_BindsACommentMadeWhileQueued(t *testing.T) {
	api := newFakeAPI()
	api.myRun = runJSON(runOpts{id: myRunID, created: "2026-09-03T10:00:00Z"})
	...
	assert.Equal(t, mustTime(t, "2026-09-03T10:00:00Z"), w.Baseline(),
		"the first delta starts where follow-ups do: at my run's creation")

	run := forgeRun(runOpts{id: myRunID + 1, event: "issue_comment", created: "2026-09-03T10:01:03Z"})
	run.Actor = "maintainer"
	require.True(t, w.isFollowUp(run))
	d, err := w.buildDelta(context.Background(), w.Baseline(), authorizedActors([]forge.WorkflowRun{run}))
	require.NoError(t, err)
	require.Len(t, d.amendments, 1, "the queued-gap comment must bind as an amendment")
	assert.Equal(t, []int64{int64(run.ID)}, d.amendments[0].RunIDs)
	assert.Empty(t, d.unbound)
}

// DeltaBaseline stays authoritative: a validation-loop retry must not
// rewind to the run's creation and re-send what the last iteration steered.
func TestFirstDelta_DeltaBaselineWins(t *testing.T) {
	api := newFakeAPI()
	api.myRun = runJSON(runOpts{id: myRunID, created: "2026-09-03T10:00:00Z"})
	w := newWatcher(t, api, &stubItems{}, &recorder{}, func(c *Config) {
		c.StartedAt = mustTime(t, "2026-09-03T10:03:00Z")
		c.DeltaBaseline = mustTime(t, "2026-09-03T10:20:00Z")
	})
	assert.Equal(t, mustTime(t, "2026-09-03T10:20:00Z"), w.Baseline())
}
...
func TestFreshAfter_FallsBackToRunnerStart(t *testing.T) {
	...
	assert.Equal(t, mustTime(t, runStart), w.Baseline())
}
