...
// into the sandbox (mailbox append, or on Codex the sandbox stop and start
// that interrupts the turn) and races the credential refreshers run.go
// already serializes through that lock, and the lock lives in internal/cli
// so the runtime cannot take it itself.
...
type Config struct {
	...
	// delta.
	SelfLogins []string
	// StartedAt is the runner's start. Start prefers the run's server-side
	// creation time as the follow-up cutoff and the first delta's baseline,
	// and falls back to StartedAt only when the run record has no usable
	// timestamp.
	StartedAt time.Time
	...
	// DeltaBaseline seeds the delta window. Zero means the run's creation
	// time, as Start resolves it (StartedAt when the record has none). The
...
func (w *Watcher) Start(ctx context.Context, stageHints ...string) error {
	...
	// The first delta starts at the same instant. A later baseline would
	// accept a queued-gap follow-up here and then filter out the comment
	// that triggered it, leaving the run unbound and its work to the
	// queued run. A validation-loop retry keeps the baseline it was given.
	if w.cfg.DeltaBaseline.IsZero() {
		w.mu.Lock()
		w.baseline = w.freshAfter
		w.mu.Unlock()
	}
