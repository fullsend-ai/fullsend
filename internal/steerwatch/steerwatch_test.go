...
func runJSON(o runOpts) map[string]any {
	...
	return map[string]any{
		...
		// On a first attempt both name the event's initiator — the comment
		// author for issue_comment; they differ only on a re-run.
...
type stubItems struct {
	...
	sinceSeen time.Time
	// compare answers CompareChanges; nil answers ErrNotFound, as the
	// forge does for a sha it cannot resolve. compared records the
	// base...head asked for.
	compare  *forge.CommitComparison
	compared string
}
...
func (s *stubItems) ListIssueCommentsSince(_ context.Context, _, _ string, _ int, since time.Time) ([]forge.IssueComment, error) {
	...
	// caller may assume — an old comment edited after since comes back.
	var out []forge.IssueComment
	for i, c := range s.comments {
		// The forge always numbers comments; a fixture that did not
		// bother gets its position, so binding by id still works.
		if c.ID == 0 {
			c.ID = i + 1
		}
		if since.IsZero() || !commentBindingTime(c).Before(since) {
			out = append(out, c)
		}
	}
	return out, nil
...
func (s *stubItems) CompareChanges(_ context.Context, _, _, base, head string) (*forge.CommitComparison, error) {
	s.compared = base + "..." + head
	if s.err != nil {
		return nil, s.err
	}
	if s.compare == nil {
		return nil, fmt.Errorf("compare: %w", forge.ErrNotFound)
	}
