...
func runFullInstall(t *testing.T, env *e2eEnv) ([]layers.AgentCredentials, *config.OrgConfig, []string, []int64) {
	...
	workflowsLayer := layers.NewWorkflowsLayer(testOrg, env.client, orgCfg, env.printer, user)
...
func runUninstall(t *testing.T, env *e2eEnv) {
	...
	stack := layers.NewStack(
		...
		layers.NewWorkflowsLayer(testOrg, env.client, nil, env.printer, ""),
...
func buildTestLayerStack(
	...
	return layers.NewStack(
		...
		layers.NewWorkflowsLayer(org, client, cfg, printer, user),
