...
// TestConcurrentAccess exercises the real gitlabci.Driver under the race
// detector with a FakeClient backend. The Driver is an immutable wrapper
// around forge.Client and a string Token — it holds no unsynchronized
// mutable fields of its own — so sharing one instance across goroutines
// is safe by design. This test verifies that property: any unsynchronized
// field access (now or added in the future) would cause -race to fire.
//
