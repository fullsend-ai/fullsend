...
| pi-anthropic-vertex extension (#6464, swapped to the fullsend-owned extension in #7019) | `PI_ANTHROPIC_VERTEX_VERSION` (git tag) + `PI_ANTHROPIC_VERTEX_SHA256` of the GitHub tag tarball; deps via `npm ci --omit=dev --omit=peer --ignore-scripts` from the vendored `package-lock.json` (26 packages — `google-auth-library` and its tree; that lockfile is **not** renovate-managed, so advisories in it surface only when the tag is bumped — run `npm audit --omit=dev --omit=peer` in the extension dir when reviewing a bump) | `sha256sum -c` + npm registry integrity check |
