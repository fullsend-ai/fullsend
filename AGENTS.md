...
- When adding a new CLI subcommand or public API endpoint, grep `docs/` for references to sibling subcommands or endpoints (e.g., when adding `mint delete`, search for `mint deploy`) and add documentation to every file that enumerates the parent command's subcommands or the API's endpoints. Pay special attention to `docs/cli/`, `docs/guides/dev/cli-internals.md`, operations guides, and infrastructure references.
