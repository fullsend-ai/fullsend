...
bootstrap:
	...
	@if [ -d "_agents" ] && [ ! -e "_agents/.git" ] && [ -f ".gitmodules" ]; then \
		echo "==> Initializing agents submodule..."; \
		git submodule update --init _agents; \
	fi
...
lint-md-links:
	lychee --offline --no-progress --include-fragments --exclude-path node_modules --exclude-path experiments --exclude-path ^_agents/ --exclude-path docs/agents/ --exclude-path docs/archived-roadmaps/2026-07.md '**/*.md'
