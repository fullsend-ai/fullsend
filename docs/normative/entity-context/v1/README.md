...
checks require `started_at`, `completed_at`, and `conclusion`.
...
- GitLab `created`, `waiting_for_resource`, `preparing`, `pending`, `scheduled`,
...
also `invalid_metadata`. Native timestamp fields forbidden for the mapped
status are omitted rather than copied.
...
thread order files, and review views contain only retained records. A reply
edge is emitted only when both endpoints remain. A thread with no retained
records is omitted from both relationship and state arrays and has no order or
view file; dangling record keys are forbidden.
...
endings, and have no byte-order mark. A non-empty body or log ends in exactly
one LF. The byte bound includes that LF. To bound non-empty text, remove all
...
code-point boundary with no trailing LF added. A truncated display string is
scanned again exactly as emitted before canonical serialization. This includes
entity titles and labels; entity and manifest-record author logins; actor
logins; check names; and agent names. Required display strings that cannot be
represented safely abort the snapshot; no placeholder or unfiltered fallback
is allowed. Redaction reduces exposure but does not promise anonymization;
...
- GitLab issue and merge-request `opened` and `closed` map to `open` and
  ...
  means its discussion is locked and maps to `open`; discussion locking is
  separate mutable state and is not represented in entity metadata v1.
