...
or to what is defanged is a new major version. The agent definitions match on
the opening line and cannot be updated atomically with the runner; the
structural tokens and the defanged set are what the runner's neutralization,
and any consumer that parses the envelope, key on. Adding a header sentence
that carries no token in the structural table is not.
