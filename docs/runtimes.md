...
| Steer (absorb a work-item update mid-run, [ADR 0113](ADRs/0113-steer-the-running-agent-on-work-item-updates.md)) | Live — the message lands at the agent's next tool boundary, same process | Live — same, over `pi --mode rpc`; not on a run that can fall back to another model | Interrupt and resume — the runner stops and restarts the sandbox, then resumes the same session with the message; each steer costs about 46 s on the pinned OpenShell 0.0.116. The stop, start and resume path was probed locally on OpenShell 0.0.116 with codex 0.157.0 |
