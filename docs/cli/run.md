...
| `token_usage` | Persisted token counts: `input`, `output`, `cache_creation`, and `cache_read` are always mutually disjoint; `reasoning` is always present too (`0` when there is none), but is disjoint from `output` only for Codex and pi — see the linked guide for the per-runtime rule. Recorded even when `total_cost_usd` is zero (cancelled runs, Codex). See [Computing dollar cost from token telemetry](../guides/infrastructure/distributed-tracing.md#computing-dollar-cost-from-token-telemetry) |
