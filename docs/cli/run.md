...
| `token_usage` | Persisted disjoint token counts (`input`, `output`, `cache_creation`, `cache_read`, and `reasoning` when present). Recorded even when `total_cost_usd` is zero (cancelled runs, Codex). See [Computing dollar cost from token telemetry](../guides/infrastructure/distributed-tracing.md#computing-dollar-cost-from-token-telemetry) |
