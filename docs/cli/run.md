...
The global run timeout is wall-clock, so a wedged agent looks exactly like a thinking one until it expires — and the run is billed for the difference. The watchdog watches the runtime output stream instead: every well-formed line the runtime writes counts as liveness, including lines that map to no agent event (pi's `tool_execution_update` while a tool streams output, Claude Code's `user` tool-result messages, codex's `item.started`/`item.completed` bracketing each command), so an actively streaming tool is never mistaken for a stall. Codex is the exception: it writes nothing between a command's `item.started` and `item.completed` (its `item.updated` is a plan/todo event, not a command stream), so a long codex command emits no intermediate lines. What keeps it from reading as silence is unified-exec returning control to the model at least every 30s (`MAX_YIELD_TIME_MS`, default-on at the pinned `CODEX_VERSION`) — each yield is a fresh tool call whose lines reset the clock. A pin that flips that default would turn a long codex command into a false stall. A pi sub-agent's output stays inside the Agent tool, so the tool emits a progress update every 30s while its child runs — a long-running persona is never mistaken for a stall either, and a child that is itself wedged is bounded by its own budget (`timeoutSeconds`, 15 minutes), not by the watchdog. It covers claude, pi and codex — every runtime that streams — and the scripted `dummy` runtimes ignore it. After half of `FULLSEND_STALL_TIMEOUT` of stream silence it warns once (`::warning::no agent events for 7m30s` in CI), and after the full duration it kills the run: first the agent inside the sandbox, through the same TERM-then-KILL sweep that clears stray processes between iterations, then the local `openshell sandbox exec` client. The global-timeout path sweeps the sandbox too — the run loop runs the same TERM-then-KILL sweep when an iteration ends at its budget, before extraction and regardless of `--keep-sandbox`. What is unique to the stall path is that it sweeps from the runtime layer, because the run loop's failure branch returns before that budget-end sweep would run; cancelling the exec only releases the local client and signals nothing inside the sandbox. The run then fails with `agent stalled` and records `"stalled": true` in `metrics.json`. The sweep is not serialized against the host-side credential refreshers the way the between-iteration sweep is, so a sandbox kept with `--keep-sandbox` after a stall kill may hold a truncated `.gcp-oidc-token` on top of an expired one: the refreshers stop with the run and the token lives five minutes, so re-seed credentials before reusing that sandbox rather than expecting the ones the run left.

`FULLSEND_STALL_TIMEOUT` takes a Go duration and defaults to `15m`; `0` disables the watchdog. The default sits above Claude Code's bash ceiling — `BASH_MAX_TIMEOUT_MS` defaults to 600000ms (10 minutes) and the model routinely requests the full ceiling for test suites — so a legitimately quiet long command is not killed as stalled; a repo that raises `BASH_MAX_TIMEOUT_MS` should raise the stall timeout with it. The value must clear the harness `timeout_minutes` by more than the watchdog's polling interval (a twentieth of the stall timeout, capped at 30s): any closer and the global timeout wins the race, so the watchdog is not armed and the run logs that stall protection is inactive. Harnesses with a short `timeout_minutes` — 15 minutes or less, since the default plus its 30s interval is 15m30s — therefore get no stall protection at the default; lower `FULLSEND_STALL_TIMEOUT` for those or accept that the global timeout is the only backstop. A non-zero value below a `1m` floor is raised to it (and the run logs so): a pi sub-agent feeds the watchdog only every 30s while a child runs, so a shorter timeout would kill a parent waiting on a healthy child. A value that is not a duration is reported in the run log and ignored, and the default applies.
