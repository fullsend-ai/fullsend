...
| Variable | Value |
|---|---|
| `FULLSEND_RUN_HEAD_SHA` | The work item's head at run start. Empty on an issue run, which has no head, and today on GitLab, where the poller-raised agent pipeline does not carry the merge request's head |
