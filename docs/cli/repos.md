...
`defaults.config` and per-repository `config` declare a sparse typed overlay for `.fullsend/config.yaml` ([ADR 0122](../ADRs/0122-declarative-repos-config-overlays.md)). They share the per-repo config schema except `runtime` and `allowed_remote_resources`, which remain the existing manifest shorthands. `defaults.config` opts every repository in; a repository `config` (including `config: {}`) opts in only that repository. Unknown fields fail validation. See [Repo Management — Configuration overlays](../guides/getting-started/repo-management.md#configuration-overlays).
