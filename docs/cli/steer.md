...
and exits 0. The run in flight finds the comment on its next poll — within
`steer.poll_interval_seconds`, 30 seconds by default — or at its next turn end,
whichever comes first. How the update then reaches the agent depends on the
runtime. Claude and pi take it into the running turn at the agent's next tool
boundary, without ending the turn. Codex stops the current turn and resumes the same session
with the update as the next prompt, which adds about 46 seconds
([steering](../contributing/steering.md#transport)).
