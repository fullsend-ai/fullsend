...
| `validating files: providers[N]: stat .../providers/....yaml: no such file or directory` | The harness names a provider file that is not on disk. Locally this is an error; CI layers `providers/` from the scaffold | Commit the file at the path the error shows. In CI, `providers/` is layered for you |
