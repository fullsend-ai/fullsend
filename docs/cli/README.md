...
| `fullsend steer <url> "<text>"` | Send an update to the agent already running on an issue or PR. Posts the stage's own command — `/fs-review` for a pull request, `/fs-triage` for an issue, or `--stage` to choose; see [ADR 0101](../ADRs/0101-steer-the-running-agent-on-work-item-updates.md). |
