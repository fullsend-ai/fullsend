...
- **Unanimous rework:** All reviewers agree changes are needed. The [fix agent](../../agents/fix.md) triggers automatically, reads the consolidated review body, and pushes fixes to the existing PR. After the fix, a new review round begins. On GitHub, bot-triggered fix is skipped when the current HEAD is a human-authored commit pushed in the last 30 minutes (use `/fs-fix` to re-engage).
