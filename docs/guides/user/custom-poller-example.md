...
Do **not** re-expose the matrix as a `workflow_dispatch` or `repository_dispatch` input, and do not copy unauthenticated external content into `matrix` without validation. Those shapes are the GitHub equivalent of the GitLab API-triggered pipeline vector that HMAC signing narrows on GitLab — verification there is gated on `PIPELINE_SOURCE`, so the risk is reduced rather than fully closed (see [ADR 0067](../../ADRs/0067-gitlab-cron-polling-event-dispatch.md)). Details: [Forged CI dispatch payloads](../../problems/security-threat-model.md#forged-ci-dispatch-payloads).
