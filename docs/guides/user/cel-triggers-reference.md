...
On GitHub, the per-repo shim drops a `commented` review submission when its
top-level body is empty. This removes the empty review containers generated by
inline replies; it also means an inline-only review with no top-level body does
not reach custom harness triggers. Non-empty `commented` reviews and other
review states continue to dispatch normally.
