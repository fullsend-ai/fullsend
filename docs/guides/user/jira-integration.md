...
**Dispatch authenticity:** The poll job and the harness job share one GitHub Actions run. The matrix is a job output passed into `reusable-dispatch.yml` through `workflow_call`, not an API-supplied pipeline variable. GitHub has no equivalent of GitLab's "create a pipeline of the trusted CI YAML with arbitrary extra variables," so this handoff does not use `FULLSEND_DISPATCH_HMAC`. Do not re-expose the matrix as a `workflow_dispatch` or `repository_dispatch` input. See [Forged CI dispatch payloads](../../problems/security-threat-model.md#forged-ci-dispatch-payloads) and [ADR 0067](../../ADRs/0067-gitlab-cron-polling-event-dispatch.md) for the GitLab contrast and the GitHub architectures that would change this.
