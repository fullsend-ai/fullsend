...
### Prohibiting Co-authored-by trailers

Some repos forbid AI attribution in commits (no `Co-authored-by` trailer, no
"Generated by Claude" footer). Write that rule in `AGENTS.md` or `CLAUDE.md`:

```markdown
## No attribution
Do not add Co-Authored-By lines, AI signatures, or any mention of Claude,
Anthropic, or AI assistance in commits, PRs, or comments.
```

Fullsend enforces the Co-authored-by half of that rule in the runner, not by
asking the model to remember it:

- Claude Code's automatic `Co-authored-by` trailer is turned off
...
(`do not add Co-authored-by`, `never add Co-Authored-By`, `no Co-authored-by`).
A repo that requires Co-authored-by, or merely mentions it, is unchanged.
...
takes precedence. Commit-attribution bans are an exception to "advisory
only": they override Claude Code's default `Co-authored-by` trailer because
that default is a runtime setting, not part of the agent definition. See
[Prohibiting Co-authored-by trailers](#prohibiting-co-authored-by-trailers).
...
- Prohibit `Co-authored-by` trailers in agent commits — the runner enforces
  this when the wording is an explicit ban (see
  [Prohibiting Co-authored-by trailers](#prohibiting-co-authored-by-trailers))
