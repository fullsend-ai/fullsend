...
the dispatching event carried, empty for an issue and today on GitLab's poller-raised pipeline) and
`FULLSEND_RUN_STARTED_AT` (RFC 3339 UTC). The start is the runner's own clock at the top of
`fullsend run`, after the concurrency wait and the job steps that precede the command, so activity
