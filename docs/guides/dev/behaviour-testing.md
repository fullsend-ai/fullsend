...
The After hook calls `Driver.DeallocateRepo` to return the slot. `Driver.Finalize` collects mint trace logs (when `BEHAVIOUR_ARTIFACT_DIR` is set), then tears down suite-scoped resources (e.g. preview mint) and reclaims outstanding leases with an error. Log collection is best-effort and does not fail the suite.
...
BEHAVIOUR_ARTIFACT_DIR=        # CI upload-artifact root for debug logs, mint trace logs, and run artifacts; temp dir when unset
