...
> **Planned deprecation.** Per-org installation mode will be deprecated in favor of per-repo installation. New installations should use the [per-repo Getting Started guides](README.md). Existing per-org installations continue to work and are fully supported during the transition.
