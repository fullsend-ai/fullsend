...
GPT models on the pi or codex runtime use OpenAI Workload Identity Federation instead — no GCP involved and no stored key on the recommended path; see [OpenAI Workload Identity](../infrastructure/openai-workload-identity.md). If you cannot enrol a WIF provider, that guide's Route C uses a `FULLSEND_OPENAI_API_KEY` repository secret. A repository that runs only GPT can skip this page entirely: declare it with `--inference-provider openai` and the GCP project and WIF provider below become optional (that guide's [section D](../infrastructure/openai-workload-identity.md#d-a-repository-with-no-gcp-project-at-all)).
