...
The dev mint is a local HTTP server that replaces the GCP token mint infrastructure (Secret Manager, Workload Identity Federation, Cloud Functions) for development and evaluation. It stores GitHub App PEM keys on disk and mints real installation tokens — no cloud infrastructure required for token minting.
...
> **Note:** The dev mint eliminates GCP for **token minting** only. Agents still need an LLM to do their work — fullsend currently uses Claude on Vertex AI, which requires a GCP project with the Vertex AI API enabled. The `--inference-project` flag during install configures this. Without it, agents will start but fail when they try to call the model.

## Prerequisites

- **fullsend CLI** (v0.14.0+ or built from source):
...
## Step 1: Start the mint (first-time setup)

On first run the data directory is empty — no `config.json` exists yet, so OIDC verification cannot initialize. Start the mint with `--insecure-no-auth` for the initial install:
...
> **First-run only.** After install populates the data directory (Step 2), you will restart the mint with `--tunnel` and OIDC enabled (Step 3).
...
Run the install command, pointing at the local mint:

```bash
fullsend admin install <org> \
  --mint-url http://localhost:8321 \
  --skip-mint-check \
  --mint-data-dir ~/.fullsend-mint \
  --app-set <org-name> \
...
| Flag | Purpose |
|------|---------|
| `--mint-url` | The mint URL. Use `http://localhost:8321` for the initial install; update to the tunnel URL in Step 3. |
| `--skip-mint-check` | Bypasses GCP mint validation; uses the dev mint instead |
| `--mint-data-dir` | Path to the dev mint's data directory (same as `--data-dir` in Step 1). PEMs and config are written here during install so the mint can serve tokens. |
...
5. Set `FULLSEND_MINT_URL` as an org variable pointing to the mint URL
...
## Step 3: Restart with tunnel

Now that the data directory is populated, stop the mint (Ctrl-C) and restart with `--tunnel` and OIDC enabled:
...
fullsend mint run \
  ...
  --port 8321 \
  --tunnel \
  --allowed-workflows '.github/workflows/dispatch.yml'
```

Replace `.github/workflows/dispatch.yml` with the actual workflow file(s) that should be allowed to mint tokens. Pass `*` to allow all workflows in the org (useful during initial testing, but restrict before sharing the tunnel URL with others).
...
Re-run the install command to update `FULLSEND_MINT_URL` to the tunnel URL:
...
The installer detects existing apps and the `.fullsend` repo, prompting you to reuse them. When prompted for PEM files for existing apps, point to the mint's stored copies at `~/.fullsend-mint/pems/<role>.pem`.

Verify the mint is reachable through the tunnel:
...
Check that the mint has all roles registered. These endpoints require OIDC Bearer tokens when the mint is running with authentication enabled (the default). For local verification, restart the mint briefly with `--insecure-no-auth` (without `--tunnel`), or use the `/health` endpoint which is always unauthenticated:

```bash
# Health check (always unauthenticated)
curl http://localhost:8321/health

# Status and token endpoints require OIDC auth in authenticated mode.
# For local testing only, restart the mint with --insecure-no-auth:
#   fullsend mint run --data-dir ~/.fullsend-mint --insecure-no-auth
curl http://localhost:8321/v1/status | jq .
```

Expected `/v1/status` output:
...
Test token minting directly (requires `--insecure-no-auth` for local curl):
...
If you restart the tunnel and the URL changes, you need to update the `FULLSEND_MINT_URL` org variable. Re-running the install command will do this:
...
The dev mint loads PEMs from the `--data-dir` directory at startup:
...
> **The dev mint is for development only.** Do not use it in production.

| Control | Dev mint | Production mint (GCP Cloud Function) |
|---------|----------------|--------------------------------------|
| OIDC verification | Enabled by default; disable with `--insecure-no-auth` | Always enabled |
| Allowed workflows | All workflows (`*`) by default | Explicit `ALLOWED_WORKFLOWS` required |
| Allowed roles | All roles by default | Explicit `ALLOWED_ROLES` required |
| PEM storage | Unencrypted files on disk | GCP Secret Manager |
| Network exposure | Loopback only (default); tunnel via cloudflared | GCP-managed HTTPS endpoint |
| `--insecure-no-auth` | Available; blocked with `--tunnel` or non-loopback `--bind` | Not available |

Use `--allowed-workflows` to restrict which GitHub Actions workflow files can request tokens when operating with OIDC enabled.

## Limitations

- **OIDC verification via JWKS** — the dev mint verifies OIDC tokens by directly fetching JWKS from GitHub's OIDC issuer, bypassing GCP Workload Identity Federation. Use `--insecure-no-auth` to disable verification for local testing only.
- **Permissive workflow and role defaults** — the dev mint allows all workflows (`*`) and all roles by default, even when OIDC is enabled. The production mint requires explicit `ALLOWED_WORKFLOWS` and role restrictions. This is an accepted trade-off for development: the dev mint is short-lived, single-org, and typically runs behind an ephemeral tunnel. For tighter controls, pass `--allowed-workflows` with explicit workflow paths.
