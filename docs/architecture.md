...
- Runtime, model, effort and persona models are routed **per task** by a `routing:` policy in `.fullsend/config.yaml`, resolved by `fullsend run` on the host before the sandbox exists, from system-derived task facts only (forge change stats and paths, actor kind, allowlisted labels, attempt count — never PR text); a rule sits below the per-run flags and variables and above the agent's `agents:` entry, is bounded by `allowed_models`, a never-downgrade set, persona floors and a served-model preflight, and is echoed in the plan block, `metrics.json`, the root span and the status comment; `shadow` mode records the decision without acting on it, and learning lands only as a reviewed policy change ([ADR 0112](ADRs/0112-task-grain-routing-policy-on-the-host.md)).
