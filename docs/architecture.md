...
- A run in flight **absorbs updates to its own work item** rather than leaving
  them to the run queued behind it: the runner watches the execution platform's
  follow-up run records, accepts one only when that run's `Route` job already
  authorized it, and delivers the delta into the live session through the
  ...
  run knows to skip. On by default; a harness opts out with
  `steer: {enabled: false}` and gets ADR 0101 alone
