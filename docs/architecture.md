...
- Coder/fix `workflows: write` is gated: triage (or human) labels the issue `fullsend-needs-workflow-write`; the code workflow assesses target-repo workflow-visible secrets (repo, org, environment names plus default `GITHUB_TOKEN` posture) against a builtin + configurable allowlist before minting; fix reuses PR label `fullsend-workflow-write` from coder or an authorized human. Implementation blocked until [ADR 0041](ADRs/0041-synchronous-workflow-call-event-dispatch.md) lands ([ADR 0044](ADRs/0044-gate-workflows-write-on-repo-secret-allowlist.md)).
