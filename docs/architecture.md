...
- Human-gated permission adjustments: elevated permissions (e.g., `workflows: write`) require explicit human authorization via forge-native labels verified by `fullsend auth check` in enrolled workflows and post-scripts. Policy (label checks, collaborator verification, stale invalidation) lives in the CLI; the mint mechanically merges requested elevations on top of the role baseline; privileged pushes execute only in deterministic automation, never in the LLM sandbox ([ADR 0074](ADRs/0074-human-gated-permission-adjustments.md)).
