...
- `fullsend-ai/platform`: Fullsend has an optional platform subsystem in the
...
and runs its own version of fullsend. The optional `fullsend-ai/platform`
deployment is a separate model: one tenant can span multiple forge
organizations, repositories, and Jira projects.
