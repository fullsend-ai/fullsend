...
- Shim `/fs-stop`: the shim `stop-agent` job applies `fullsend-no-*` labels to skip auto agent triggers; dispatch enforces those labels on auto paths while `/fs-*` on-demand still works. Author escape hatch is fix-only ([ADR 0086](ADRs/0086-fs-stop-agent-and-no-agent-labels.md)).
