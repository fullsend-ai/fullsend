...
- Semantic observability and improvement loop: host-side derived signals enrich traces from [ADR 0050](ADRs/0050-distributed-tracing-instrumentation.md) with fullsend-specific patterns (OTel-primary; Claude `stream-json` optional); a read-first observer analyzes traces/signals without replacing the retro agent; shadow mode gates future observer forge writes; structured lesson extraction feeds human-reviewed lessons into eval golden sets ([ADR 0082](ADRs/0082-semantic-observability-and-improvement-loop.md)).

**Open questions:**

- What signals matter most — cost, latency, token usage, action logs, decision traces, or something else? (Derived signals / fullsend-specific pattern markers partially decided in [ADR 0082](ADRs/0082-semantic-observability-and-improvement-loop.md); cost and latency remain on the tracing layer; which signals matter most operationally remains open.)
...
- Is there a real-time monitoring requirement (agent is stuck, agent is behaving anomalously), or is observability primarily forensic? (v1 observer post-run / derived stuck-run heuristics / optional v2 mid-pipeline scorecard partially decided in [ADR 0082](ADRs/0082-semantic-observability-and-improvement-loop.md); true real-time in-sandbox monitoring remains open.)
