...
- Layered content resolution: upstream defaults (agents, skills, schemas,
  ...
  sparse-checkout of `fullsend-ai/fullsend@<sha>` (pinned at install time). The scaffold installs only
...
| Agent runner | GitHub Actions job → `fullsend run` CLI (via `fullsend-ai/fullsend@<sha>` composite action, SHA-pinned at install time) | |
