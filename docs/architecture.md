...
- Event-driven stage dispatch: `dispatch.yml` invokes stage workflows via synchronous `workflow_call` instead of async `workflow_dispatch` + `gh workflow run`, so the dispatched run stays linked to the caller in the GitHub Actions UI ([ADR 0041](ADRs/0041-synchronous-workflow-call-event-dispatch.md)). This changes the nesting arithmetic from ADR 0031 — the full synchronous chain (shim → dispatch → thin caller → reusable) uses 3 of the 10 available `workflow_call` nesting levels (50 calls per run). Under ADR 0031's async model, `workflow_dispatch` reset the nesting counter; under ADR 0041's sync model, nesting accumulates. Thin callers remain valuable despite consuming a nesting level: they provide explicit `secrets:` passthrough (least-privilege over `secrets: inherit`) and `vars.*` → `inputs.*` mapping that makes the reusable workflow's parameter contract visible and auditable ([ADR 0031](ADRs/0031-reusable-workflows-for-action-installed-distribution.md)).
