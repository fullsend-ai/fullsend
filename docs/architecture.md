...
- Runtime resolution: `fullsend run <name>` resolves agents in two tiers: (1) config entries from `OrgConfig.Agents` (highest priority), (2) runtime fallback to the `fullsend-ai/agents` repository for known first-party agents not in config. A GitHub 401/403 (other than a rate limit) while resolving that ref is fatal — the CLI does not warn-and-continue, because a rejected PAT otherwise masks the root cause behind later unrelated failures. The agents-repo fallback is a transitional mechanism for the agent extraction; it will be removed once all users have migrated to config-driven registration (ADR 0058 Phase 5).
