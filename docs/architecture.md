...
- Jira project enrollment: `fullsend jira enroll` CLI command configures inbound event path (Jira Automation → `repository_dispatch`) and outbound credentials (Jira API token as GitHub secret). Existing agents gain Jira awareness via harness composition, not new agents. Repo-to-issue association is handled separately by the poll driver design ([ADR 0069](ADRs/0069-jira-project-enrollment-via-cli.md)).
