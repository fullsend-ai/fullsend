...
- a push whose review round is **skipped** (draft, `fullsend-no-review`, documentation prose, pusher below triage — [ADR 0096](ADRs/0096-skip-provably-unnecessary-review-dispatch.md)) clears the same two labels from the per-repo dispatch workflow, so the invariant holds whether or not a round starts (the deprecated per-org scaffold has no such step)
