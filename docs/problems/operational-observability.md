...
- ~~How do we measure "is the system getting better"? What metrics constitute a meaningful quality signal for an autonomous software factory? Merge revert rate? Human override rate? Time-to-review? Cost per decision? Some composite score? The choice of metric shapes what gets optimized.~~ Partially decided in [ADR 0082](../ADRs/0082-semantic-observability-and-improvement-loop.md): structured lesson extraction from retro/observe output creates reviewed, testable memory that feeds eval golden sets; composite operational metrics remain open.
...
- ~~Should observability data feed back into agent instructions automatically (e.g., auto-adjusting prompts when false positive rates exceed a threshold), or should it only inform human-driven instruction changes? Automatic feedback creates the risk of instruction oscillation; human-only feedback is slower but more controlled.~~ Partially decided in [ADR 0082](../ADRs/0082-semantic-observability-and-improvement-loop.md): lessons flow through human-reviewed PRs before becoming eval cases; observer write actions are shadowed by default; automatic prompt adjustment remains out of scope.
