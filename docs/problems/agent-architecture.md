...
- Should there be one instance of each agent type per repo, per org, or shared? Per-repo is simpler but more expensive. Shared agents need careful isolation. The optional managed platform establishes the tenant as an administrative boundary, while execution topology and isolation remain open ([ADR 0110](../ADRs/0110-managed-tenant-platform.md); infrastructure constrains this — see [agent-infrastructure.md](agent-infrastructure.md).)
