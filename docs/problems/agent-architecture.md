...
- Should there be one instance of each agent type per repo, per org, or shared? Per-repo is simpler but more expensive. Shared agents need careful isolation. If a centrally managed service is offered, its per-tenant repository configuration will be GitOps-managed; execution topology and isolation remain open ([ADR 0123](../ADRs/0123-gitops-managed-tenant-configuration.md); infrastructure constrains this — see [agent-infrastructure.md](agent-infrastructure.md).)
