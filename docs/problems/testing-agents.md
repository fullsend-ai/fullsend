...
- **Production outcome tracking** — when a human overrides an agent's decision, log it as a potential test case. If the agent approved something a human rejected, that's a signal. Structured lesson extraction from retro and observe runs ([ADR 0082](../ADRs/0082-semantic-observability-and-improvement-loop.md)) provides a reviewed path from production signals to golden-set cases.
