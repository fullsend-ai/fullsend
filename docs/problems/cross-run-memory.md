...
- How should memory interact with separate agent roles? Should triage memory ever influence review behavior directly? (Same-agent JSONL resume is [ADR 0092](../ADRs/0092-resume-agent-sessions-from-jsonl-transcripts.md); cross-role transcript consumption is still this question.)
