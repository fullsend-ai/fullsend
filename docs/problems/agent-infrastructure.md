...
- Who in the org would own and operate agent infrastructure, and how does that align with existing platform or CI ownership? [ADR 0123](../ADRs/0123-gitops-managed-tenant-configuration.md) establishes GitOps-managed `repos.yaml` configuration for a possible centralized offering; whether to offer the service, its operational ownership, and its infrastructure remain open.
