...
- **Is the forge abstraction worth the cost at this stage?** Fullsend's only concrete *implementation* is GitHub, but [ADR 0043](../ADRs/0043-gitlab-support-via-webhook-bridge.md) and its companion [implementation plan](../plans/gitlab-support.md) define a complete second-forge architecture for GitLab. The question shifts from "is forge-neutrality premature?" to "does the current abstraction layer adequately serve both forges?" — i.e., whether the existing `forge.Client` interface cleanly accommodates GitLab's PAT model, webhook bridge, and OIDC flow without requiring platform-specific branches in shared code.
