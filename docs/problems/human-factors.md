...
- **Review comments lose authority.** A review comment from a known expert carries weight because readers know the person has context and judgment. An agent-generated review comment may be technically accurate but lacks the relational trust that makes review feedback actionable. Contributors may dismiss agent feedback they would accept from a peer. The damage is worse when the comment is not merely untrusted but *factually false about input the agent already had* — claiming the PR body omits a change the body lists (see [the grounding problem](code-review.md#the-grounding-problem-fabricated-claims-about-the-pr-description) in code review). One such finding teaches reviewers to treat the rest of the review as noise.
...
- **Trustworthiness evidence** builds trust in agent actions through structured evidence. The voice concern here is the complementary problem: trust in agent communication.
