...
- **Tiered response based on actor trust** — write (or, for observation stages, triage) permission is already required before slash-command or issue/PR-open dispatch can trigger an agent ([ADR 0054](../ADRs/0054-require-authorization-on-all-agent-dispatch-paths.md)), so non-collaborators cannot trigger those paths today. One documented exception: `pull_request_target.closed` → retro is intentionally left ungated, so any PR closer — including an untrusted external author closing their own PR — can trigger retro without a permission check. This is not read-only lifecycle accounting: retro runs with a minted token that has `issues:write` and `pull_requests:write` while contents remains read-only; its post-script can file issues, comment, and apply `ready-for-triage`, which the router dispatches to triage without another actor-authorization check. An untrusted PR closer can therefore force retro inference and indirectly trigger a downstream triage run, leaving a residual DOS/injection surface. See also [run-trigger authorization](platform-nativeness.md#run-trigger-authorization-the-pwn-request-gap), which treats this ungated path as the exception to fullsend's collaborator-permission dispatch gate. On top of the live permission check on gated paths, a reputation-based tier — accounts that are new or have thin contribution history — could still be subject to stricter rate limits or require human approval before triggering agents. This is a starting point, not a complete defense: [Threat 7](#threat-7-coordinated-inauthentic-contributions) specifically games the reputation-building process this control relies on.
