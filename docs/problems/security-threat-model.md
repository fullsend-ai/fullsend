...
- **Event debouncing and deduplication** — preserve one active invocation and
  ...
  absorbs an update instead receipts it, and the queued run exits on that receipt. The receipt is
  authenticated by an author the sandbox cannot post as, and no receipt is honoured while a
  provider hands the sandbox that token, so the agent cannot forge one to drop an update
