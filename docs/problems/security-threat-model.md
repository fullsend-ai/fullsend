...
In fullsend, agents cannot take forge actions directly — credentialed operations (push, label, comment) are applied outside the sandbox ([ADR 0017](../ADRs/0017-credential-isolation-for-sandboxed-agents.md)). Today, deterministic post-scripts constrain agents to the operations those scripts implement. [ADR 0093](../ADRs/0093-reuse-gh-aw-safe-outputs-for-github-side-effects.md) migrates GitHub writes to gh-aw safe outputs, which add narrower permissions, explicit operation declarations, validation, and limits. This limits the blast radius, but the agent still controls the *content* of its output (the diff it produces, the triage decision it makes), and output application applies validated output faithfully. [ADR 0022](../ADRs/0022-harness-level-output-schema-enforcement.md) validates output structure but not semantic proportionality — it cannot distinguish a proportionate one-file fix from an unnecessary forty-file rewrite triggered by an emotionally charged comment.
