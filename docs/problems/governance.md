...
- How do we handle the bootstrap problem — who sets up the initial agent configuration for a new repo, and how is that initial setup secured? (Preset-based install and `config.base.yaml` / `config.yaml` layering decided in [ADR 0063](../ADRs/0063-ready-made-configuration-presets.md); workflow pinning and backend policy remain open.)
