...
- **Autonomy levels** — which repos are agent-autonomous, which are in shadow mode, which require full human review. What are the graduation criteria, and who evaluates them? (Observer-initiated forge-write shadowing is a narrower mechanism partially decided in [ADR 0082](../ADRs/0082-semantic-observability-and-improvement-loop.md); repo-level autonomy graduation remains open.)
