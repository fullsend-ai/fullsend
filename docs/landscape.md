...
[GitHub](https://github.com/openai/symphony) | [SPEC.md](https://github.com/openai/symphony/blob/main/SPEC.md) | [Elixir reference](https://github.com/openai/symphony/blob/main/elixir/README.md)

OpenAI's framework for turning tracker work into isolated, autonomous implementation runs. A long-running orchestrator polls an issue tracker, claims tasks, and dispatches a Codex app-server session per issue. The README markets "proof of work" (CI status, PR review feedback, complexity analysis, walkthrough videos) and a handoff where engineers accept work rather than supervising the coding agent.

Pinned to [`8001b52`](https://github.com/openai/symphony/commit/8001b52e3062) (2026-08-12, latest on `main` at time of writing). Apache 2.0. ~27k stars / ~2.8k forks as of 2026-09-09. Issues are disabled; two tagged releases (`v0.0.1`, `v0.0.2`) ship Burrito-packaged Elixir binaries. The README frames this as "a low-key engineering preview for testing in trusted environments"; the Elixir tree is labeled "prototype software intended for evaluation only." The distinctive distribution model is spec-as-product: give `SPEC.md` to a coding agent and have it reimplement.

**Architecture:** Symphony is a central daemon. Quoting [SPEC Section 7](https://github.com/openai/symphony/blob/main/SPEC.md#7-orchestration-state-machine): "The orchestrator is the only component that mutates scheduling state. All worker outcomes are reported back to it and converted into explicit state transitions." State lives in memory; restart recovery is tracker- and filesystem-driven, with no durable database (a SPEC TODO). The orchestrator owns the poll loop, claim/running/retry maps, concurrency limits, reconciliation, and dispatch. Workers report structured events; Symphony itself is a tracker *reader* — ticket mutations are performed by the coding agent through provider-native tools ([SPEC Section 11.5](https://github.com/openai/symphony/blob/main/SPEC.md#115-tracker-writes-and-agent-tools-important-boundary)). The Elixir reference implements adapters for Linear, GitHub Issues, Jira Cloud, Asana, and GitLab behind a generic tracker interface; the coding-agent integration remains Codex app-server only.

This is the largest architectural divergence from fullsend. Per [agent-architecture.md](problems/agent-architecture.md#interaction-model-the-repo-as-coordinator), fullsend uses the repository's permission model as coordinator — no agent orchestrates other agents. Branch protection, CODEOWNERS, required status checks, and forge events provide coordination. There is no in-process scheduling state to lose and no orchestrator to attack. The trade-off is portability versus attack surface: Symphony's daemon ports to any tracker once an adapter exists; fullsend eliminates a single attack/failure target by binding more deeply to one forge's permission model.

Against other entries here: [Gas City](#gas-town-gas-city) also has a controller process, but enforces Zero Framework Cognition so the controller contains zero judgment. Symphony's orchestrator visibly contains judgment in candidate selection (priority sort, blocker rules) and active-run reconciliation — measurably more cognition in the controller than Gas City's stated discipline.

**Trust and security:** The spec is explicitly non-prescriptive about trust posture. [Section 15.1](https://github.com/openai/symphony/blob/main/SPEC.md#151-trust-boundary-assumption): "Each implementation defines its own trust boundary." Non-goals include mandating strong sandbox controls beyond what the coding agent and host OS provide, and mandating a single default approval/sandbox posture. [Section 15.5](https://github.com/openai/symphony/blob/main/SPEC.md#155-harness-hardening-guidance) warns that a permissive deployment can lead to data leaks, destructive mutations, or full machine compromise, then declines to mandate a hardening posture. Hooks are fully trusted configuration ([Section 15.4](https://github.com/openai/symphony/blob/main/SPEC.md#154-hook-script-safety)).
...
- *Trust between agents.* Symphony is one agent per issue. There is no inter-agent communication, so Threat 5 is moot by construction. Fullsend pays a defense-in-depth cost because review is decomposed across independent sub-agents ([agent-architecture.md](problems/agent-architecture.md#review-sub-agents)).
- *External prompt injection.* Not addressed. Tracker text is rendered into the prompt template via Liquid with no sanitization layer, no canary tripwires, and no separation of trusted instructions from untrusted tracker content. A malicious comment is sufficient. Compare [Threat 1](problems/security-threat-model.md#threat-1-external-prompt-injection).
...
**Proof of work:** None of the marketed artifacts (CI status, PR review, complexity analysis, walkthrough videos) are produced by Symphony itself. [SPEC Section 13](https://github.com/openai/symphony/blob/main/SPEC.md#13-logging-status-and-observability) specifies structured logs, an optional runtime snapshot, optional humanized event summaries, and an optional HTTP server. The richer artifacts live in `WORKFLOW.md` and in-repo Codex skills (`commit`, `debug`, `land`, tracker skills) — the agent assembles a narrative bundle at handoff. Fullsend's proof is a set of independent verdicts the merging agent did not produce ([two-phase review](problems/agent-architecture.md#two-phase-review-model)). CI-as-gate (required status checks) is structurally stronger than CI-as-narrative. Self-review collapses to the agent grading itself; the [MAGIS finding](problems/agent-architecture.md#ideas-to-consider-borrowing) that removing the QA agent cut performance by more than half cuts against that.

Two observability pieces are worth borrowing: [SPEC Section 13.5](https://github.com/openai/symphony/blob/main/SPEC.md#135-session-metrics-and-token-accounting)'s token-accounting rules ("prefer absolute thread totals; ignore delta-style payloads as cumulative totals"), and complexity analysis as a candidate review concern currently absent from fullsend's [code-review.md](problems/code-review.md) decomposition. Walkthrough videos are for human consumption and conflict with autonomous-merge as a target; they may be useful at the human-approval boundary for higher intent-authorization tiers ([intent-representation.md](problems/intent-representation.md)) but not as autonomous-flow infrastructure.

**Governance:** No formal approval gate. [SPEC Section 14.4](https://github.com/openai/symphony/blob/main/SPEC.md#144-operator-intervention-points) names operator intervention points (kill a worker, restart the service) that are operational, not authorization. A run ends when the workflow's handoff state is reached, and the agent is the one transitioning the tracker. There is no CODEOWNERS-equivalent and no required-status-check equivalent. The Elixir demo uses Linear statuses "Rework", "Human Review", "Merging" — workflow conventions, not authorization primitives. Fullsend derives blocking authority from approval rights on a path ([agent-architecture.md](problems/agent-architecture.md#core-principle-trust-derives-from-repository-permissions-not-agent-identity)).
...
Not reusable: Codex app-server protocol coupling, the in-memory orchestrator, and treating tracker writes as agent-tool side effects. These conflict with repo-as-coordinator and [forge abstraction](ADRs/0005-forge-abstraction-layer.md). Elixir is a sensible BEAM choice for Symphony's design; it is not a Rosetta Stone for a Go port ([ADR 0004](ADRs/0004-go-as-implementation-language.md)).

**Comparison:**

| Dimension | Symphony | Fullsend direction |
|---|---|---|
| Coordinator | Central daemon (cognitive) | Repository as coordinator |
| Tracker | Linear, GitHub Issues, Jira, Asana, GitLab | Forge-neutral `forge.Client` |
| Runtime | Codex app-server only | Multiple runtimes |
| Isolation | cwd discipline + RECOMMENDED OS hardening | Per-role sandbox + credential isolation |
| Mandated sandbox | No | Yes |
| Review | One agent per issue, self-assembled proof | Independent zero-trust review sub-agents |
| Merge authority | Workflow convention / human handoff | CODEOWNERS + required checks |
| Threat model | Implementation-defined | Ranked, documented, load-bearing |
| Distinguishing strength | Spec-as-product, formal state machine | Repo-visible authority, security-as-foundation |

**Relevance to fullsend:** Symphony is a **study reference, not an adoption candidate**. It is the closest published artifact to fullsend's problem space, but its architectural choices land on the opposite side of every load-bearing fullsend principle: central orchestrator vs repo-as-coordinator, implementation-defined trust vs security-as-foundation, cwd-discipline vs sandbox-as-containment, single-agent-with-self-proof vs decomposed independent review.
