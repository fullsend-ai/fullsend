...
| unified-exec yield default (`MAX_YIELD_TIME_MS` = 30s; the `unified_exec` feature `Stage::Stable`, `default_enabled: true` at 0.152.1) | it is the stall watchdog's only codex liveness during a long command — nothing streams between a command's `item.started` and `item.completed`, so a long command relies on unified-exec handing control back to the model at least every 30s; a pin that raised the yield or flipped the feature off would make every long codex command a false stall | `codex-rs/core/src/unified_exec/mod.rs`, `codex-rs/features/src/lib.rs` |
