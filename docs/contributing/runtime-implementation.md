...
`runtime.appendHookEnv` writes into `/sandbox/workspace/.env`; the runtime must launch the scripts with that file sourced (Claude's run command does). Findings go to `/sandbox/workspace/.security/findings.jsonl`.
