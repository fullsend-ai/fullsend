...
**Unicode** strips invisible, bidi, tag, NUL and ANSI/OSC characters and runs of variation selectors, but keeps compatibility characters (fullwidth, ligatures, CJK punctuation) and single emoji/CJK selectors. Stripping runs to a fixpoint so a single non-overlapping substitution cannot leave a newly adjacent CSI/OSC sequence. NFKC is applied to a *detection copy* (canary, secret patterns); a field is emitted normalized only when the normalized copy reveals a recognized control/invisible payload or a secret the original hid. After that rewrite, every sanitizer category is re-checked to a fixpoint, so the emitted field cannot retain a sequence reconstructed by NFKC after the last scan. The fixpoint loop is bounded by a fixed pass budget (defense against pathological input); if that budget is ever exhausted before convergence, the sanitizer fails closed by force-stripping every remaining ESC/C1 byte rather than emitting text that could still carry a live CSI/OSC sequence. Decoded tag-character payloads are recorded only in `findings.jsonl` (bounded) and never in tool output or the agent transcript. Host-side `UnicodeNormalizer` (input/output pipelines, trajectory redaction) applies NFKC unconditionally and uses the same post-NFKC fixpoint so forge-bound and trajectory text cannot carry a reconstructed control sequence either.
