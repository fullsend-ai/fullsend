...
| `personas` | Per-persona model/tools table, keyed by persona name. Present when `sub-agents/*.md` files were discovered under skill roots. The extension dispatches a `subagent_type` that names a persona using its resolved model and tool set; an unknown non-empty type is rejected when personas are registered. Model resolution: repo `subagents.<persona>` > `subagents.default` > frontmatter `model:` > parent. Each spec is canonicalised and checked against the trusted set before it enters the manifest (#7031) |
