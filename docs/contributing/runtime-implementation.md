...
| `pi-anthropic-vertex` | `ARG PI_ANTHROPIC_VERTEX_VERSION` + tarball SHA256, under `/usr/local/share/pi-extensions/anthropic-vertex` | the extension's own CI matrix against the pinned `PI_VERSION` — it wraps pi's Anthropic transport and mirrors no internals, so there is no `sync/compat.json` or SDK-override check; then its `peerDependencies` floor (declared only — `--omit=peer` never installs it) |
...
pi's `google-vertex` provider is Gemini-only and the upstream `anthropic-vertex` provider is an open PR (earendil-works/pi#5262, still open as of 2026-09-04). The sandbox image vendors [`fullsend-ai/pi-anthropic-vertex`](https://github.com/fullsend-ai/pi-anthropic-vertex) v0.1.0 (MIT, fullsend-owned) under `/usr/local/share/pi-extensions/anthropic-vertex`, pinned by tag + tarball SHA256 (`PI_ANTHROPIC_VERTEX_VERSION`/`_SHA256`). It registers provider `anthropic-vertex` whose models are pi's own first-party Claude catalog re-pointed at Vertex: pi's built-in Anthropic transport is used untouched, and a request-rewriting `fetch` does what the Vertex SDK does — the model id moves into the URL path (`/v1/projects/<project>/locations/<region>/publishers/anthropic/models/<model>:{streamRawPredict|rawPredict}`), `anthropic_version` goes into the body, and the `x-api-key` pi passes the Google token in becomes an `authorization: Bearer`. The endpoint host is the extension's own (`{region}-aiplatform.googleapis.com`, or the `global`/`us`/`eu` forms), derived from the resolved region rather than from any `ANTHROPIC_*` variable — note the region itself still comes from `CLOUD_ML_REGION`/`GOOGLE_CLOUD_LOCATION` and is interpolated into the hostname, and the run does not scrub those. It carries no `@anthropic-ai/*` dependency, so there is no SDK version to keep aligned with pi's. It is root-owned and outside `PI_CODING_AGENT_DIR`, so pi never auto-loads it; for the `anthropic-vertex` provider `Run` passes it with `-e` (`runtime.piVertexExtensionPath`; providers without a vendored extension get pi's built-ins only).

It replaced [`twoGiants/pi-anthropic-vertex`](https://github.com/twoGiants/pi-anthropic-vertex) v0.1.13 (#7019), which mirrored ~220 lines of pi's `streamSimple` helpers and pinned `@anthropic-ai/sdk` underneath a client cast — so every pi bump was a drift risk, its `sync/compat.json` stopped at pi 0.81.1, and from pi 0.84.3 it hard-failed `claude-opus-5` with `400 fallbacks: Extra inputs are not permitted`.
...
**Credential hygiene — keep the unset.** The extension bundles no Anthropic SDK, reads no `ANTHROPIC_*` variable except `ANTHROPIC_VERTEX_PROJECT_ID` (its third project fallback), and pins its own endpoint host, so it can no longer be steered by `ANTHROPIC_API_KEY` or `ANTHROPIC_VERTEX_BASE_URL` the way the bundled Vertex client in the old extension could. That removes the *original* reason for the scrub but not the scrub: pi's built-in `anthropic` provider resolves its credential from `ANTHROPIC_AUTH_TOKEN`, then `ANTHROPIC_OAUTH_TOKEN`/`ANTHROPIC_API_KEY`, beneath any extension, so a stray value still has somewhere to land if a spec ever resolves to that provider. (`ANTHROPIC_BASE_URL` is scrubbed for a different reason: pi passes `baseURL: model.baseUrl` for every built-in Anthropic model, so the SDK's env fallback only applies to a model defined without one.) `Run` therefore still unsets `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_BASE_URL`/`ANTHROPIC_VERTEX_BASE_URL` after sourcing `.env` for the `anthropic-vertex` provider (matched case-insensitively, as pi resolves provider prefixes) and keeps them for a direct `anthropic` provider, which needs the key; `childEnv` in `fullsend-agent.js` applies the same rules per resolved child provider. Both lists predate `ANTHROPIC_OAUTH_TOKEN` being one of the three names pi resolves, so that one is not scrubbed today — worth closing, and bounded meanwhile by the `*.googleapis.com` egress allowlist. Treat the scrub as defense in depth, not dead code.

**Re-check on every `PI_VERSION` or extension bump:** the extension's CI matrix (`.github/workflows/ci.yml` in its own repo) runs the suite against fullsend's pinned `PI_VERSION` and the latest pi release on every push to its `main` and every PR, and a tag is cut from a main commit that passed it — so that matrix is the compatibility check, and there is no `sync/compat.json` to consult and no SDK override to align. (The tag job itself runs `npm ci`, which resolves the peer from the vendored lockfile, so it exercises only the newer leg.) It mirrors no pi internals, so a pi release can only reach it through the public `anthropicMessagesApi()` / `createProvider` / `getModels` surface, which is what those tests exercise. On a `PI_VERSION` bump, move the matrix's older entry to the new pin first, then confirm the extension's `peerDependencies` floor still covers it — `npm ci --omit=peer` never installs the peer, so that floor is a declaration to read, not a constraint the build enforces. One drift surface does survive the wrap-based design: the extension forwards pi's `compat` flags through its own `VERTEX_COMPAT_KEYS` allowlist, so a compat key a new pi release adds is dropped silently on Vertex rather than erroring (the allowlist is `satisfies readonly (keyof AnthropicMessagesCompat)[]`, which checks the listed keys are real but never demands the list be exhaustive, so its own `tsc` run stays green too). On a `PI_VERSION` bump, diff pi's `AnthropicMessagesCompat` against that allowlist and decide explicitly about anything new — and note the drop is not always silent: `getAnthropicCompat` fills every key with a default and several default to `true`, so a new defaulted-on key can still put its field on the wire. Vertex can also still reject a request shape pi introduces: smoke an adaptive and a non-adaptive model against Vertex on a bump and override in `PI_CODING_AGENT_DIR/models.json` rather than patching the extension.
