...
| `runtime.Steerer` | Optional — delivers a mid-run message into a running session, and settles it ([steering.md](steering.md)). Consulted only when `RunParams.Steerable` is set; `Run` then returns after `Settle` and the agent's current turn. **The caller must hold the runner's sandbox lock across every `Steer` and `Settle`**: both write into the running sandbox and race the credential refreshers the runner serializes through it. Omit and the runtime runs single-turn, exactly as it does today |
| `runtime.SteerDecliner` | Optional, on a `Steerer` — `SteerDeclineReason` refuses a run before the runner starts its watcher, and the refusal is announced whatever the harness says ([steering.md](steering.md#the-steer-contract)). Pi declines a run whose model chain can fall back |
