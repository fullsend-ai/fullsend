...
- **Where that trade bites.** The duplicate-poll case is the least comfortable one: two duplicate dispatches are the same work rather than a newer state superseding an older one, so both run to completion instead of one replacing the other.
