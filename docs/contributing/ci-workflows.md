...
- **Agent stage jobs never cancel the run in flight.** Every stage job in `reusable-dispatch.yml` carries `cancel-in-progress: false`. A newer event on the same work item leaves the run already working on it alone; GitHub keeps one pending run per group, the last to queue, normally but not necessarily the newest event, which then works from the item's current state rather than its own event ([ADR 0113](../ADRs/0113-preserve-the-agent-run-in-flight-on-work-item-updates.md)). Keep the value identical across every stage job — one role cancelling while another queues on the same item is harder to reason about than either behaviour on its own. The trade: [ADR 0063](../ADRs/0063-polling-based-work-discovery.md) counted per-stage cancellation as a layer against duplicate poll dispatch, and that layer is gone, leaving the poller's lock and agent idempotency.
- **What setting it gives up.** [ADR 0063](../ADRs/0063-polling-based-work-discovery.md) counts per-stage `cancel-in-progress` as the second of two layers protecting poll dispatch from duplicate side effects, so preserving runs leaves the poller's own lock and whatever idempotency the agent has. The duplicate-poll case is the least comfortable one: two duplicate dispatches are the same work rather than a newer state superseding an older one, so both run to completion instead of one replacing the other.
