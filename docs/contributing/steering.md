...
- [Transport: follow-up runs are the requests](#transport-follow-up-runs-are-the-requests)
...
- [The skip check](#the-skip-check) — the receipt the queued run reads
- [The fleet-agent backstop](#the-fleet-agent-backstop)
- [Configuration](#configuration)
- [What changes for each stage](#what-changes-for-each-stage)
...
`Route` job already applied [ADR 0054](../ADRs/0054-require-authorization-on-all-agent-dispatch-paths.md)'s authorization. That
...
`issue_comment` (see [Amendments and context](#amendments-and-context) above). So the runner needs
...
steerable run) and Codex's `turn.completed` all normalize to — the watcher polls once. If something new arrived it steers
...
be idempotent, which [ADR 0063](../ADRs/0063-polling-based-work-discovery.md) already asks for. Code stops
