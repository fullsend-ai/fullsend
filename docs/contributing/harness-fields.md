...
| `steer`            | Follow-up run watcher settings are operational, not forge-specific (ADR-0113) |
...
| `steer`          | Child replaces base entirely (if non-nil)            | Absent (nil) = inherit the base's whole block, including an `enabled: false` opt-out; a child setting `steer:` gets the defaults for any key it omits, not the base's values |
