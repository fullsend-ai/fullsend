...
Codex exec has no live channel. The runner stops the sandbox, which ends the turn and keeps the disk
state, starts it again, and resumes the same session with the message as the next prompt. The
interface is therefore two methods rather than a single send.

Stop is the Codex interrupt because an exec's processes are not expected to exit with the caller
([NVIDIA/OpenShell#3159](https://github.com/NVIDIA/OpenShell/issues/3159)), and `sandbox stop` is the
released way to end them. On the pinned OpenShell 0.0.116 a stop takes 45 seconds
([NVIDIA/OpenShell#2855](https://github.com/NVIDIA/OpenShell/issues/2855)). The fix
([NVIDIA/OpenShell#3036](https://github.com/NVIDIA/OpenShell/pull/3036)) ships in OpenShell v0.1.0,
where a stop measured 0.25 to 0.48 seconds on the v0.1.0-pre.12 build. fullsend's CI still pins
0.0.116, so the 45-second cost holds until a separate bump moves the pin.
...
- The live runtimes need no new sandbox operation: `exec` and `upload` are the whole transport. The
  Codex interrupt adds `sandbox stop` and `start`, and costs about 46 seconds per steer on the
  pinned OpenShell 0.0.116.
