...
The playlist entry's branch policy is explicit data in the entry, not an inference from the entry name. This policy is part of the playlist contract and applies equally to built-in and custom agents. On each invocation the runtime:
...
3. If the entry contains no `repo/` files, no branch operation occurs. If it contains `repo/` files, `branch: feature` creates and commits to a new feature branch, while `branch: current` commits to the checked-out branch. `branch: none` is invalid when `repo/` files are present. `branch: none` is the default when omitted.
4. If `.fullsend/playback-comment-url` exists and can be read, the runtime uses the forge tracking comment as the durable playlist position. After serving the entry, it advances the local playlist and best-effort updates that tracking comment. If the locator is absent or cannot be read, the runtime uses the local playlist position instead. Tracking-comment update failures are warnings; an invalid position from a readable tracking comment is an error.
