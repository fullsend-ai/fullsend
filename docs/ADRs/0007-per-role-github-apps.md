...
- **Coder workflow elevation:** The coder app manifest includes `workflows: write` as a ceiling permission; mint grants it only via the `workflow-change` authorization gate ([ADR 0054](0054-label-gated-elevated-agent-permissions.md)). Default coder tokens remain without `workflows: write`.
