...
The cost is a second place where harness shape is encoded. That is mitigated
but not eliminated: the harness is built as a `harness.Harness` value and
marshalled rather than formatted as text, so the generator cannot emit a field
the validator does not know about; golden tests pin the generated bytes; and
the role table has a drift test against the mint. A new *required* harness
field would still need a matching change here, and the golden tests are what
would catch it.

Two limits are deliberate and documented. `ValidateRunnerEnvWith` is not run at
generation time — it requires every `${VAR}` in the harness `env` blocks to be
set in the calling process, which is true in CI and false on a developer's
machine — so an unset variable still surfaces at `fullsend run`. And the
default sandbox image digests are compiled in, so they are repinned by hand on
the same cadence as the fleet repin PRs; a golden test makes a repin visible in
review.
