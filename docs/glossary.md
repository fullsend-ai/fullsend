...
`/fs-fix-stop`, etc., that either manually triggers an agent workflow or
controls auto-triggers (for example `/fs-stop` / `/fs-fix-stop` apply
`fullsend-no-*` skip labels). The `/fs-` prefix
